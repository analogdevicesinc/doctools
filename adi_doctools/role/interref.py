...
Changes:
* References to repos in the lut.
* No implicit resolution, equivalent to intersphinx_disabled_reftypes = ['*']
* Allow empty suffix, defaults to ref, e.g.
  :ref-hdl:`spi_engine axi` is equal to :ref-hdl:ref:`spi_engine axi`
* Don't allow any inventory search (removed resolve_reference_any_inventory),
...
* Projects using the interref extension can specify repos to resolve to links
...
class InventoryAdapter:
    ...
    def __init__(self, env: BuildEnvironment) -> None:
        ...
        if not hasattr(env, 'interref_cache'):
            # initial storage when fetching inventories before processing
            self.env.interref_cache = {}  # type: ignore[attr-defined]

            self.env.interref_inventory = {}  # type: ignore[attr-defined]
            self.env.interref_named_inventory = {}  # type: ignore[attr-defined]
    ...
    def cache(self) -> dict[str, InventoryCacheEntry]:
        ...
        - Element one is the key given in the Sphinx interref_mapping
          configuration value
        - Element two is a time value for cache invalidation, a float
        - Element three is the loaded remote inventory, type Inventory
        """
        return self.env.interref_cache  # type: ignore[attr-defined]

    @property
    def main_inventory(self) -> Inventory:
        return self.env.interref_inventory  # type: ignore[attr-defined]

    @property
    def named_inventory(self) -> dict[str, Inventory]:
        return self.env.interref_named_inventory  # type: ignore[attr-defined]

    def clear(self) -> None:
        self.env.interref_inventory.clear()  # type: ignore[attr-defined]
        self.env.interref_named_inventory.clear()  # type: ignore[attr-defined]
...
def fetch_inventory(app: Sphinx, uri: str, inv: str) -> Inventory:
    ...
    # *inv* can be local or remote URIs
    try:
        if '://' in inv:
...
def fetch_inventory_group(
    ...
    cache_time = now - app.config.interref_cache_limit * 86400
    failures = []
    try:
        for inv in invs:
            if not inv:
...
def load_mappings(app: Sphinx) -> None:
    ...
    interref_cache: dict[str, InventoryCacheEntry] = inventories.cache
    ...
    with concurrent.futures.ThreadPoolExecutor() as pool:
        ...
        for name, (uri, invs) in app.config.interref_mapping.values():
            futures.append(pool.submit(
                fetch_inventory_group, name, uri, invs, interref_cache, app, now,
            ))
    ...
    if any(updated):
        ...
        named_vals = []
        unnamed_vals = []
        for name, _expiry, invdata in interref_cache.values():
            if name:
...
def _create_element_from_result(domain: Domain, inv_name: str | None,
                                ...
                                base_uri: str) -> nodes.reference:
    proj, version, uri, dispname = data
    if '://' not in uri and node.get('refdoc'):
        # replace path to hosted with single depth '../'
...
def _resolve_reference_in_domain_by_target(
    ...
    for objtype in objtypes:
        ...
        if target in inventory[objtype]:
            ...
            # other types.
            continue
        return _create_element_from_result(domain, inv_name, data, node, contnode, uri)
    return None
...
def _resolve_reference_in_domain(env: BuildEnvironment,
    ...
    res = _resolve_reference_in_domain_by_target(inv_name, inventory, domain, objtypes,
                                                 node['reftarget'], node, contnode, interref_uri)
    if res is not None:
        return res
    ...
    return _resolve_reference_in_domain_by_target(inv_name, inventory, domain, objtypes,
                                                  full_qualified_name, node, contnode, interref_uri)
...
class IntersphinxRole(SphinxRole):
    ...
    def run(self) -> tuple[list[Node], list[system_message]]:
        ...
        if role_name is None:
            logger.warning(__('role for cross-reference not found: %s'), name_suffix,
    ...
    def run(self, **kwargs: Any) -> None:
        for node in self.document.findall(pending_xref):
            ...
            if inv_name is not None:
                ...
                if newnode is None:
                    typ = node['reftype']
                    msg = (__('ref %s:%s reference target not found: %s') %
                ...
                typ = node['reftype']
                msg = (__('ref %s:%s %s  without inventory name is not allowed.') %
                       (node['refdomain'], typ, node['reftarget']))
                logger.warning(msg, location=node, type='ref', subtype=typ)
                node.replace_self(contnode)
...
def normalize_interref_mapping(app: Sphinx, config: Config) -> None:
    ...
    for key, value in config.interref_mapping.copy().items():
        try:
            if isinstance(value, (list, tuple)):
                ...
                if not isinstance(name, str):
                    logger.warning(__('intersphinx identifier %r is not string. Ignored'),
                                   name)
                    config.interref_mapping.pop(key)
                    continue
            else:
                msg = (
                    "Wrong interref_mapping entry format."
                    f"Hint: \"interref_mapping = {{'<name>': {(uri, inv)!r}}}\"."
                )
                logger.warning(msg)
                config.interref_mapping.pop(key)
                continue
            ...
            if not isinstance(inv, tuple):
                config.interref_mapping[key] = (name, (uri, (inv,)))
            else:
                config.interref_mapping[key] = (name, (uri, inv))
        except Exception as exc:
            logger.warning(__('Failed to read interref_mapping[%s], ignored: %r'), key, exc)
            config.interref_mapping.pop(key)


def interref_setup(app: Sphinx) -> dict[str, Any]:
    app.add_config_value('interref_mapping', {}, 'env')
    app.add_config_value('interref_repos', [], 'env')
    app.add_config_value('interref_cache_limit', 5, '')
    app.add_config_value('interref_timeout', None, '')

    app.connect('config-inited', normalize_interref_mapping, priority=800)
