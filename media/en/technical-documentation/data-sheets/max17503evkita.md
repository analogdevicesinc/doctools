...
The MAX17503 3.3V output evaluation kit (EV kit) provides a proven design to evaluate the MAX17503 high-voltage, high-efficiency,  synchronous  step-down  DC-DC converter. The EV kit is preset for 3.3V output at load currents up to 2.5A and features a 450kHz switching frequency for optimum  efficiency  and  component  size.  The  EV  kit features  adjustable  input  undervoltage  lockout,  adjustable  soft-start,  open-drain RESET signal,  and  external frequency synchronization.
...
## Evaluates: MAX17503 in 3.3V Output-Voltage Application
...
| DESIGNATION   |   QTY | DESCRIPTION                                                           |
|---------------|-------|-----------------------------------------------------------------------|
| L1            |     1 | 6.8µH, 5A inductor Coilcraft MSS1048-682NL Taiyo Yuden NS10165T6R8NNA |
...
| TP1, TP2      |     2 | Test pads                                                             |
| U1            |     1 | Buck converter (20 TQFN-EP*) Maxim MAX17503ATP+                       |
| -             |     3 | Shunts (JU1, JU2, JU3)                                                |
| -             |     1 | PCB: MAX17503 - 3.3V Output EVKIT                                     |

Note: C7, R1, and R2 are optional components; R1 and R2 are not needed if the EN/UVLO pin is permanently connected to VIN. The electrolytic capacitor (C7) is required only when the VIN power supply is situated far from the MAX17503-based circuit. When R5 is open, the device switches at 500kHz switching frequency.
...
The  MAX17503  3.3V  output  EV  kit  provides  a  proven design  to  evaluate  the  MAX17503  high-voltage,  highefficiency, synchronous step-down DC-DC converter. The EV	kit	is	preset	for	3.3V	output	from	5V	to	60V	input	at load currents up to 2.5A and features a 450kHz switching frequency for optimum efficiency and component size.
...
| SHUNT POSITION   | EN/UVLO PIN                                                | MAX17503_ OUTPUT                                        |
|------------------|------------------------------------------------------------|---------------------------------------------------------|
...
changes	 on	 the	 MODE	 pin	 are	 ignored	 during	 normal operation.	Refer	to	the	MAX17503	IC	data	sheet	for	more information	on	PWM,	PFM,	and	DCM	modes	of	operation.
...
| SHUNT POSITION   | MODE PIN          | MAX17503_ MODE        |
|------------------|-------------------|-----------------------|
...
| SHUNT POSITION   | SYNC PIN                      | MAX17503_ SYNC                                       |
|------------------|-------------------------------|------------------------------------------------------|
...
Figure 1. MAX17503 3.3V Output Load and Line Regulation (PWM Mode)
...
Figure 3. MAX17503 3.3V Output Load and Line Regulation (PFM Mode)
...
Figure 5. MAX17503 3.3V Output Efficiency (DCM Mode)

Figure 6. MAX17503 3.3V Output Full Load Bode Plot (VIN = 24V)

<!-- image -->

Figure 7. MAX17503 3.3V Output, No Load to 1A Load Transient (PWM Mode)

<!-- image -->

## EV Kit Test Report (continued)

Figure 8. MAX17503 3.3V Output, 5mA to 1A Load Transient (PFM Mode)

<!-- image -->

Figure 9. MAX17503 3.3V Output, 50mA to 1A Load Transient (DCM Mode)
...
Figure 12. MAX17503 3.3V Output EV Kit Component Placement Guide-Component Side

<!-- image -->

Figure 13. MAX17503 3.3V Output EV Kit Component Side PCB layout

<!-- image -->

## Evaluates: MAX17503 in 3.3V Output-Voltage Application

Figure 14. MAX17503 3.3V Output EV Kit PCB LayoutInner Layer 1

<!-- image -->

Figure 15. MAX17503 3.3V Output EV Kit PCB Layout-Inner Layer 2

<!-- image -->

## MAX17503 3.3V Output Evaluation Kit

Figure 16. MAX17503 3.3V Output EV Kit PCB LayoutSolder Side

<!-- image -->

## Evaluates: MAX17503 in 3.3V Output-Voltage Application

Figure 17. MAX17503 3.3V Output EV Kit Component Placement Guide-Top Solder Mask

<!-- image -->

Figure 18. MAX17503 3.3V Output EV Kit Component Placement Guide-Bottom Solder Mask
