...
|-----------------|----------------|----------------|
| DS2502-E48+     | -40°C to +85°C | 3 TO-92        |
| DS2502P-E48+    | -40°C to +85°C | 6 TSOC         |
...
| (UNUSED)     | CRC16 COMPANY ID   | CRC16 COMPANY ID         | VALUE                 | EXTENSION ID VALUE   | PROJECT ID   | LENGTH     |
|--------------|--------------------|--------------------------|-----------------------|----------------------|--------------|------------|
| (UNUSED)     | MSB LSB            | MSB                      | LSB MSB               | LSB MSB              | LSB          | LENGTH     |
| 19 BYTES FFh | 2 BYTES            | 3 BYTES CONSTANT 006035h | 3 BYTES SERIALIZATION | 4 BYTES 00001129h    | CONSTANT     | 1 BYTE 0Ah |
...
Assume  that  a  manufacturer's  company  ID  value  is  006035h  and  the  24-bit  extension  identifier  is 67ABCDh. The 48-bit node address value generated from these two numbers is 00603567ABCDh, whose byte and bit representations are illustrated in Figure 2.

Figure 2. SAMPLE NODE ADDRESS VALUE

| MOST SIGNIFICANT BYTE LEAST SIGNIFICANT BYTE   | MOST SIGNIFICANT BYTE LEAST SIGNIFICANT BYTE   | MOST SIGNIFICANT BYTE LEAST SIGNIFICANT BYTE   | MOST SIGNIFICANT BYTE LEAST SIGNIFICANT BYTE   | MOST SIGNIFICANT BYTE LEAST SIGNIFICANT BYTE   | MOST SIGNIFICANT BYTE LEAST SIGNIFICANT BYTE   |
|------------------------------------------------|------------------------------------------------|------------------------------------------------|------------------------------------------------|------------------------------------------------|------------------------------------------------|
| 00                                             | 60                                             | 35                                             | 67                                             | AB                                             | HEX                                            |
| 0000 0000                                      | 0110 0000                                      | 0011 0101                                      | 0110 0111                                      | 1010 1011                                      |                                                |
| MOST SIGNIFICANT BIT LEAST SIGNIFICANT BIT     | MOST SIGNIFICANT BIT LEAST SIGNIFICANT BIT     | MOST SIGNIFICANT BIT LEAST SIGNIFICANT BIT     | MOST SIGNIFICANT BIT LEAST SIGNIFICANT BIT     | MOST SIGNIFICANT BIT LEAST SIGNIFICANT BIT     | MOST SIGNIFICANT BIT LEAST SIGNIFICANT BIT     |
...
|-----------|------|------|------|------|------|------|------|------|------|------|------|------|------|
| DATA      | 8D   | DD   |   00 |   60 |   35 |   67 | AB   | CD   |   00 |   00 |   11 |   29 | 0A   |

The four bytes at memory addresses 01h to 04h contain the UniqueWare Project ID 00001129h. The two bytes at addresses 0Bh and 0Ch are the inverted 16-bit CRC over the length byte, Project ID and node address value. The least significant byte of the CRC is stored at address 0Bh.  This CRC is generated according  to  the  standardized  CRC16  polynomial  function  X 16 +  X 15 +  X 2 +  1.  For  more  details  on generating  CRC  values  including  examples  in  both  hardware  and  software,  see  Application  Note  27, Understanding and Using Cyclic Redundancy Checks with Maxim iButton Products .

The contents of the memory address range 0Dh to 1Fh is FFh. These cells cannot be altered since the whole  memory  page  is  write-protected.  The  memory  range  from  20h  to  7Fh,  however,  is  userprogrammable. It can be write-protected by programming the corresponding write-protect bit in the status memory of the DS2502-E48.
...
| 12/09           | Deleted paragraph on how to set up a UniqueWare project.            | 2               |
| 12/09           | Inserted actual CRC value to Figure 3.                              | 2               |
| 12/09           | Reformatted Figures 1 to 3.                                         | 2               |
| 12/09           | Corrected notation of hexadecimal numbers from H to h.              | 1, 2, 3         |