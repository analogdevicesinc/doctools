...
The MAX7313 I 2 C-compatible serial interfaced peripheral  provides microprocessors with 16 I/O ports. Each I/O port can be individually configured as either an open-drain current-sinking output rated at 50mA and 5.5V, or a logic input with transition detection. A 17th port can be used for transition detection interrupt, or as a  general-purpose output. The outputs are capable of driving LEDs, or providing logic outputs with external resistive pullup up to 5.5V.
...
The  MAX7313  is  controlled  through  the  2-wire I 2 C/SMBus serial interface, and can be configured to any one of 64 I 2 C addresses.
...
- ♦ 1.2 µ A (typ), 3.6 µ A (max) Standby Current
...
LCD Backlights LED Status Indication Portable Equipment Keypad Backlights RGB LED Drivers
...
| Supply Current (Interface Idle, PWM Enabled)          | I +         | SCL and SDAat V+; other digital inputs atV+ or GND; PWMintensity control enabled  | T A = +25°C          |          |   8.5 | 15.1     | µA      |
| Supply Current (Interface Idle, PWM Enabled)          | I +         | SCL and SDAat V+; other digital inputs atV+ or GND; PWMintensity control enabled  | T A = -40°C to +85°C |          |       | 16.5     | µA      |
| Supply Current (Interface Idle, PWM Enabled)          | I +         | SCL and SDAat V+; other digital inputs atV+ or GND; PWMintensity control enabled  | T A = T MIN to T MAX |          |       | 17.2     | µA      |
| Supply Current (Interface Running, PWM Disabled)      | I +         | f SCL = 400kHz; other digital inputs at V+ or GND; PWM intensity control disabled | T A = +25°C          |          |    50 | 95.3     | µA      |
| Supply Current (Interface Running, PWM Disabled)      | I +         | f SCL = 400kHz; other digital inputs at V+ or GND; PWM intensity control disabled | T A = -40°C to +85°C |          |       | 99.2     | µA      |
| Supply Current (Interface Running, PWM Disabled)      | I +         | f SCL = 400kHz; other digital inputs at V+ or GND; PWM intensity control disabled | T A = T MIN to T MAX |          |       | 102.4    | µA      |
...
| Input High Voltage SDA, SCL, AD0, AD1, P0-P15         | V IH        | AD2,                                                                              | AD2,                 | 0.7 ✕ V+ |       |          | V       |
| Input Low Voltage SDA, SCL, AD0, AD1, P0-P15          | V IL        | AD2,                                                                              | AD2,                 |          |       | 0.3 ✕ V+ | V       |
| Input Leakage Current SDA, SCL, AD0, AD1, AD2, P0-P15 | I IH , I IL | Input = GND or V+                                                                 | Input = GND or V+    | -0.2     |       | +0.2     | µA      |
| Input Capacitance SDA, SCL, AD0, AD1, AD2, P0-P15     |             |                                                                                   |                      | 8        |     8 | 8        | pF      |
...
|-------------------------------------|----------|--------------------------|----------------------|-------|-------|---------|
| Output Low Voltage P0-P15, INT /O16 | V OL     | V+ = 2V, I SINK = 20mA   | T A = +25°C          |  0.15 |  0.26 | V       |
...
| Output Low Voltage P0-P15, INT /O16 |          | V+ = 2.5V, I SINK = 20mA | T A = +25°C          |  0.13 |  0.23 | V       |
| Output Low Voltage P0-P15, INT /O16 |          | V+ = 2.5V, I SINK = 20mA | T A = -40°C to +85°C |       |  0.26 | V       |
| Output Low Voltage P0-P15, INT /O16 |          | V+ = 2.5V, I SINK = 20mA | T A = T MIN to T MAX |       |  0.28 | V       |
| Output Low Voltage P0-P15, INT /O16 |          | V+ = 3.3V, I SINK = 20mA | T A = +25°C          |  0.12 |  0.23 | V       |
| Output Low Voltage P0-P15, INT /O16 |          | V+ = 3.3V, I SINK = 20mA | T A = -40°C to +85°C |       |  0.24 | V       |
| Output Low Voltage P0-P15, INT /O16 |          | V+ = 3.3V, I SINK = 20mA | T A = T MIN to T MAX |       |  0.26 | V       |
...
| Fall Time of SDA Transmitting                      | t F.TX    | (Notes 2, 3, 5) |       | 20 + 0.1C b |   250 | ns      |
| Pulse Width of Spike Suppressed                    | t SP      | (Notes 2, 6)    |       | 50          |       | ns      |
...
Table 1. MAX7313 I 2 C Slave Address Map
...
## Table 1. MAX7313 I 2 C Slave Address Map (continued)
...
The MAX7313 is read using the MAX7313's internally stored command byte as an address pointer the same way the stored command byte is used as an address pointer for a write. The pointer autoincrements after each data byte is read using the same rules as for a write (Table 2). Thus, a read is initiated by first configuring the MAX7313's command byte by performing a write (Figure 7). The master can now read n consecu- tive  bytes  from  the  MAX7313 with the first data byte being read from the register addressed by the initialized command byte. When performing read-after-write verification,  remember to reset the command byte's address because the stored command byte address has been autoincremented after the write (Table 2). A diagram of a read from the input ports registers is shown in Figure 10 reflecting the states of the ports.
...
| CONFIGURATION                  | R/ W | 0x0F           | INTERRUPT STATUS | -               | INTERRUPT OUTPUT CONTROL AS GPO | INTERRUPT OUTPUT CONTROL AS GPO | INTERRUPT ENABLE | GLOBAL INTENSITY | BLINK FLIP      | BLINK ENABLE    |
| Write device configuration     | 0    | 0x0F           | INT              | X               | O1                              | O0                              | I                | G                | B               | E               |
| Read back device configuration | 1    | 0x0F           | INT              | O               | O1                              | O0                              | I                | G                | B               | E               |
| Disable blink                  | -    | 0x0F           | X                | X               | X                               | X                               | X                | X                | X               | 0               |
| Enable blink                   | -    | 0x0F           | X                | X               | X                               | X                               | X                | X                | X               | 1               |
| Flip blink register (see text) | -    | 0x0F           | X                | X               | X                               | X                               | X                | X                | 0               | 1               |
...
| Disable global intensity control-intensity is set by registers 0x10-0x17 for ports P0 through P15 when configured as outputs, and by D3-D0 of register 0x0E for INT /O16 when INT /O16 pin is configured as an output port | -    | X                | X               | X                               | X                               | X                | 0                | X               | X               |
...
| Disable data change interrupt- INT /O16 output is controlled by the O0 and O1 bits                                                                                                                                         | -    | X                | X               | X                               | X                               | 0                | X                | X               | X               |
| Enable data change interrupt- INT /O16 output is controlled by port input data change                                                                                                                                      | -    | X                | X               | X                               | X                               | 1                | X                | X               | X               |
| INT /O16 output is low (blink is disabled)                                                                                                                                                                                 | -    | X                | X               | X                               | 0                               | 0                | X                | X               | 0               |
| INT /O16 output is high impedance (blink is disabled)                                                                                                                                                                      | -    | X                | X               | X                               | 1                               | 0                | X                | X               | 0               |
| INT /O16 output is low during blink phase 0                                                                                                                                                                                | -    | X                | X               | X                               | 0                               | 0                | X                | X               | 1               |
| INT /O16 output is high impedance during blink phase 0                                                                                                                                                                     | -    | X                | X               | X                               | 1                               | 0                | X                | X               | 1               |
| INT /O16 output is low during blink phase 1                                                                                                                                                                                | -    | X                | X               | 0                               | X                               | 0                | X                | X               | 1               |
| INT /O16 output is high impedance during blink phase 1                                                                                                                                                                     | -    | X                | X               | 1                               | X                               | 0                | X                | X               | 1               |
...
If  blinking  is  enabled,  then  both  interrupt  output  controls O0 and O1 set the logic state of the INT /O16 pin according to the blink phase. PWM intensity control for O16 is set by the 4 global intensity bits in the master and O16 intensity register (Table 13).
...
|                     1 | 0                   | Enabled          | Blink phase 0 registers |
|                     1 | 1                   | Enabled          | Blink phase 1 registers |
...
| Write outputs P15-P8 phase 1     |      0 | 0x03           | OP15            | OP14            | OP13            | OP12            | OP11            | OP10            | OP9             | OP8             |
...
blink phases (Tables 8 and 9). The effect of setting an output's blink phase x register bit to 0 or 1 is shown in Table 12. LEDs can be flipped between either directly on and off, or between a variety of high/low PWM intensities.
...
I/O  ports  P0-P15, interrupt output INT /016, and serial interface SDA, SCL, AD0-2 remain high impedance with up to 6V asserted on them when the MAX7313 is powered down (V+ = 0V). The MAX7313 can therefore be used in hot-swap applications.
...
|----------------------------------------|-------------------------------|-------------------------------|---------------------------------------|---------------------------------------|--------------------------------------------------------------------|--------------------------------------------------------------------|
| OUTPUT (OR GLOBAL) INTENSITY SETTING   | PHASE X REGISTER BIT = 0      | PHASE X REGISTER BIT = 0      | OUTPUT BLINK PHASE X REGISTER BIT = 1 | OUTPUT BLINK PHASE X REGISTER BIT = 1 | BLINK PHASE 0 REGISTER BIT = 0 BLINK PHASE 1 REGISTER BIT = 1      | BLINK PHASE 0 REGISTER BIT = 1 BLINK PHASE 1 REGISTER BIT = 0      |
| OUTPUT (OR GLOBAL) INTENSITY SETTING   | LOW TIME                      | HIGH TIME                     | LOW TIME                              | HIGH TIME                             | BLINK PHASE 0 REGISTER BIT = 0 BLINK PHASE 1 REGISTER BIT = 1      | BLINK PHASE 0 REGISTER BIT = 1 BLINK PHASE 1 REGISTER BIT = 0      |
...
| Write output P3, P2 intensity                                        | 0      | 0x11 |                             | P3I2                        | P3I1                        |                             | P2I3                        | P2I2                        | P2I1                        |                             |
| Read back output P3, P2 intensity                                    | 1      | 0x11 | P3I3                        |                             |                             | P3I0                        |                             |                             |                             | P2I0                        |
...
| Write output P9, P8 intensity       | 0    | 0x14           | P9I3                                           | P9I2                                           | P911                                           | P9I0                                           | P8I3                                           | P812                                           | P811                                           | P810                                           |
...
| Read back output P11, P10 intensity | 1    | 0x15           | MSB LSB OUTPUT P13 INTENSITY                   | MSB LSB OUTPUT P13 INTENSITY                   | MSB LSB OUTPUT P13 INTENSITY                   | MSB LSB OUTPUT P13 INTENSITY                   | MSB LSB OUTPUT P12 INTENSITY                   | MSB LSB OUTPUT P12 INTENSITY                   | MSB LSB OUTPUT P12 INTENSITY                   | MSB LSB OUTPUT P12 INTENSITY                   |
| Write output P13, P12 intensity     | 0    | 0x16           | P13I3                                          | P13I2                                          | P13I1                                          | P13I0                                          | P12I3                                          | P12I2                                          | P12I1                                          | P12I0                                          |
| Read back output P13, P12 intensity | 1    | 0x15           | P13I3                                          | P13I2                                          | P13I1                                          | P13I0                                          | P12I3                                          | P12I2                                          | P12I1                                          | P12I0                                          |
...
| Write output P15, P14intensity      | 0    | 0x17           | P15I3                                          | P15I2                                          | P15I1                                          | P15I0                                          | P14I3                                          | P14I2                                          | P14I1                                          | P14I0                                          |
...
| No register                          | 0x0E       | Master, O16 intensity register                                           | Not implemented                            | Power-up default disables the blink and intensity (PWM) features           |
| No register                          | 0x0F       | Configuration register                                                   | Not implemented                            | Power-up default disables the blink and intensity (PWM) features           |
| No registers                         | 0x10-0x17  | Outputs intensity registers                                              | Not implemented                            | Power-up default disables the blink and intensity (PWM) features           |
...
The MAX7313 operates with a power-supply voltage of 2V to 3.6V. Bypass the power supply to GND with at least 0.047µF as close to the device as possible.
