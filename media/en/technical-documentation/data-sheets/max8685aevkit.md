...
| PART           | TYPE   |
|----------------|--------|
| MAX8685AEVKIT+ | EV Kit |

## Component List

| DESIGNATION   |   QTY | DESCRIPTION                                                                       |
|---------------|-------|-----------------------------------------------------------------------------------|
| C6            |     1 | 1000pF ±5%, 50V X5R ceramic capacitor (0402) Murata GRM1555C1H102J or equivalent  |
| C7            |     0 | Not installed                                                                     |
| C8            |     1 | 3300pF ±10%, 50V X5R ceramic capacitor (0402) Murata GRM155R71H332K or equivalent |
| C9            |     1 | 100pF ±5%, 50V C0G ceramic capacitor (0402) Murata GRM1555C1H101J or equivalent   |
| D1            |     1 | Yellow LED (0603) Panasonic LNJ412K84RA                                           |

1

## MAX8685A Evaluation Kit
...
| FT1           |     1 | Trim xenon 1.5 FTA PerkinElmer DTS Trim Xel.SFTA                                                                                |
| JU1, JU2      |     2 | 2-pin headers 1 x 36-pin header, 0.1in centers (comes in 1 x 36-pin strips, cut to fit) Sullins PEC36SAAN Digi-Key S1012E-36-ND |
...
| R5            |     1 | 75k Ω ±1% resistor (0402), lead free                                                                                            |
| R6, R14       |     2 | 402 Ω ±1% resistors (0402), lead free                                                                                           |

| DESIGNATION   |   QTY | DESCRIPTION                                                                              |
|---------------|-------|------------------------------------------------------------------------------------------|
| R7A, R7B      |     2 | 499k Ω ±1% resistors (1206), lead free                                                   |
| R8-R13        |     6 | 24.9k Ω ±1% resistors (1206), lead free                                                  |
| R15           |     1 | 0 Ω resistor-short (0402), lead free                                                     |
| R16           |     1 | 10 Ω ±1% resistor (0402), lead free                                                      |
| R17           |     1 | 10 Ω ±1% resistor (0402), lead free                                                      |
...
| T1            |     1 | Transformer TDK LDT565630T-011 Tokyo Coil TTRN-SUZ0S-00 1T                               |
| T2            |     1 | Transformer Tokyo Coil TS-F45B-1                                                         |
| U1            |     1 | MAX8685AETD+ (14-pin TDFN-EP, 3mm x 3mm)                                                 |
| -             |     1 | PCB: MAX8685A Evaluation Kit+                                                            |
...
Indicate that you are using the MAX8685A when contacting these component suppliers.
...
The MAX8685A EV kit is fully assembled and tested. Follow the steps below to verify board operation.

## Caution: Lethal voltages are present in this circuit.

- 1) Before connecting the EV kit, verify that the output capacitor is discharged. To discharge the output capacitor, set SW2 to the discharge position (Figure 1). Wait at least 30 seconds before proceeding.
- 2) Verify  that  SW1  is  in  the  disable  position  and  that the shunts are shorting JU1 and JU2.
...
- 7) Connect the positive terminal of the voltmeter capable of reading at least 300V to OUT.
...
| JUMPER   | FUNCTION                                                                                                                                                                                                                  |
|----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| JU1      | Shorts V CC (IC supply for MAX8685A EV kit) to BATT (transformer primary supply). Short JU1 for single-supply 2.5V to 5.5V operation. Open JU1 for separate V CC (2.5V to 5.5V) and BATT (1.5V to 10V) supplies.          |
| JU2      | Shorts V CC (IC supply for MAX8685A EV kit) to V CCT (IGBT supply for MAX8685A EV kit). Short JU2 for single- supply 2.5V to 5.5V operation. Open JU2 for separate V CC (2.5V to 5.5V) and V CCT (2.5V to 5.5V) supplies. |

## Detailed Description

## VCC, VCCT, and BATT Supplies

For single-supply operation, install the shunts for JU1 and JU2 and connect the 2.5V to 5.5V supply to the pad on the EV kit labeled BATT. When using separate supplies to power the IC and the transformer primary, remove the shunt from JU1. Connect the 2.5V to 5.5V power supply to the EV kit pad labeled VCC and connect the 1.5V to 10V transformer power supply to the EV kit pad labeled BATT.

## Adjusting the Output Voltage

The output voltage is set by the ratio of a resistor voltagedivider. Choose R3 less than 2k Ω . The EV kit comes with a 1k Ω resistor installed. Larger resistor values, combined with parasitic capacitance at FB, can slow the rise time of the FB voltage during each cycle. This may prevent the MAX8685A from detecting when the output voltage has reached the desired level.
...
The MAX8685A provides an integrated insulated gate bipolar transistor (IGBT) driver for discharging the photoflash capacitor through a xenon flash tube. The MAX8685A provides a separate power input (VCCT) for the IGBT driver.
...
If  the  MAX8685A EV kit is to be used with another xenon flash tube, ensure proper polarities are observed upon installation (Figure 2).

## Setting the UVI Threshold

The MAX8685A EV kit comes with the UVI threshold preset to 2V. To adjust the UVI threshold, replace R5 with a resistance between 37.5k Ω and 675k Ω .  Refer to the MAX8685A-MAX8685F IC data sheet for information on UVI and selecting the value for R5.

## Setting the Current Limit

The MAX8685A EV kit comes with the primary current l i mit  set  to  the  default  value  of  2A  (2.6A for the MAX8685F). To set the current limit to a lower value, remove R15 and install resistor R4. Refer to the MAX8685A-MAX8685F IC data sheet for information on current limit and selecting the value for R4.

## Output Voltage Monitoring

The voltage monitor (MTR) provides a scaled replica of the output voltage in real time. The sample-and-hold circuitry provides a voltage proportional to the output voltage. MTR provides a 2V output when VFB equals 1.25V. The voltage-monitor output is only valid when the part is charging. MTR is internally grounded in shutdown.
...
An LED and resistor are connected between VCC and DONE to  indicate when the charge cycle is complete. To use DONE as a logic-level output instead of driving an LED, remove resistor R6 and D1 from the EV kit and install a pullup resistor in R6 and a 0 Ω resistor for D1. If the flash tube has been fired, or after 16 seconds of automatic refresh mode, DONE does not assert a second time. DONE remains asserted until SW1 is toggled. DONE is not a safety indicator.
...
## Evaluating the MAX8685F

To evaluate the MAX8685F on the MAX8685A EV kit, replace the installed IC with a MAX8685F. Note that the default current limit increases to 2.6A when the MAX8685F is installed.

<!-- image -->

Figure 2. MAX8685A EV Kit Schematic for Single-Supply Operation
...
Figure 3. MAX8685A EV Kit Component Placement Guide-Component Side
...
Figure 4. MAX8685A EV Kit PCB Layout-Component Side
...
Figure 5. MAX8685A EV Kit PCB Layout-Solder Side
