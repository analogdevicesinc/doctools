...
| Input-Referred Noise (Note 4)               |          | CIN = 0.85pF, BW = 933MHz                                                    | CIN = 0.85pF, BW = 933MHz                                                    |   200 |       |       |         |
| Input-Referred Noise (Note 4)               |          | CIN = 0.85pF, BW = 2.1GHz (Note 3)                                           | CIN = 0.85pF, BW = 2.1GHz (Note 3)                                           |       |   360 |   485 |         |
| Input-Referred Noise (Note 4)               |          | CIN = 0.85pF, BW = 18GHz (Note 3)                                            | CIN = 0.85pF, BW = 18GHz (Note 3)                                            |       |       |   700 |         |
| Input-Referred Noise (Note 4)               | I N      | CIN = 0.6pF, BW = 933MHz                                                     | CIN = 0.6pF, BW = 933MHz                                                     |   200 |       |       | nA RMS  |
| Input-Referred Noise (Note 4)               |          | CIN = 0.6pF, BW = 2.1GHz (Note 3)                                            | CIN = 0.6pF, BW = 2.1GHz (Note 3)                                            |       |   325 |   440 |         |
| Input-Referred Noise (Note 4)               |          | CIN = 0.6pF, BW = 18GHz (Note 3)                                             | CIN = 0.6pF, BW = 18GHz (Note 3)                                             |       |       |   635 |         |
| Differential Transimpedance                 |          | Differential output, I IN = 40µA AVE                                         | Differential output, I IN = 40µA AVE                                         |   2.8 |   3.5 |   4.5 | k Ω     |
| Small-Signal Bandwidth (Note 3)             | BW       | -3dB, CIN = 0.6pF                                                            | -3dB, CIN = 0.6pF                                                            |     2 |   2.5 |       | GHz     |
|                                             |          | -3dB, CIN = 0.85pF                                                           | -3dB, CIN = 0.85pF                                                           |   1.8 |   2.3 |       |         |
| Low-Frequency Cutoff                        |          | -3dB, input current = 20µA AVE (Note 3)                                      | -3dB, input current = 20µA AVE (Note 3)                                      |       |       |    30 | kHz     |
| Deterministic Jitter (Notes 3, 5)           |          | 100µA P-P < input ≤                                                          | 2.1Gbps, K28.5 pattern                                                       |       |    11 |    33 | ps P-P  |
| Deterministic Jitter (Notes 3, 5)           |          | 100µA P-P < input ≤                                                          | 2.7Gbps, 231 - 1 pattern                                                     |       |    15 |       | ps P-P  |
| Deterministic Jitter (Notes 3, 5)           | DJ       | 10µA P-P < input ≤ 100µA P-P                                                 | 2.1Gbps, K28.5 pattern                                                       |       |     8 |    17 |         |
| Deterministic Jitter (Notes 3, 5)           |          | 10µA P-P < input ≤ 100µA P-P                                                 | 2.7Gbps, 231 - 1 pattern                                                     |       |    12 |       |         |
...
| PARAMETER                                                | SYMBOL   | CONDITIONS                                                                   | CONDITIONS                                                                   |   MIN |   TYP |   MAX | UNITS   |
|----------------------------------------------------------|----------|------------------------------------------------------------------------------|------------------------------------------------------------------------------|-------|-------|-------|---------|
| Single-Ended Output Common- Mode Minimum Level (MAX3724) |          | Relative to V CC , I IN = 1mA AVE                                            | Relative to V CC , I IN = 1mA AVE                                            |   540 |   490 |       | mV      |
| Output Data Transition Time                              |          | Input > 200µA P-P 20% to 80% rise/fall time (Note 3)                         | Input > 200µA P-P 20% to 80% rise/fall time (Note 3)                         |       |    70 |   100 | ps      |
| Differential Output Return Loss                          |          | Frequency ≤ 1GHz                                                             | Frequency ≤ 1GHz                                                             |       |    17 |       | dB      |
|                                                          |          | 1GHz < frequency ≤ 2GHz                                                      | 1GHz < frequency ≤ 2GHz                                                      |       |    10 |       | dB      |
| Power-Supply Noise Rejection                             | PSNR     | I IN = 0                                                                     | f < 1MHz                                                                     |       |    46 |       | dB      |
| Power-Supply Noise Rejection                             | PSNR     | I IN = 0                                                                     | 1MHz ≤ f < 10MHz                                                             |       |    34 |       | dB      |
| RSSI Gain (MAX3724)                                      | A RSSI   | (Note 7)                                                                     | (Note 7)                                                                     |       |    21 |       | A/A     |
| RSSI Gain Stability (MAX3724)                            |          | 10log(A RSSI /A RSSI-NOM ) where A RSSI-NOM = A RSSI at 3.3V, +25°C (Note 3) | 10log(A RSSI /A RSSI-NOM ) where A RSSI-NOM = A RSSI at 3.3V, +25°C (Note 3) |       |  0.24 |       | dB      |
...
| 8                           | OUT-   | Inverting Data Output. Current flowing into IN causes the voltage at OUT- to decrease. For the MAX3724, the common mode between OUT+ and OUT- is proportional to the average input current.         |
...
The output buffer provides a reverse-terminated voltage output. The buffer is designed to drive a 100 Ω differential load between OUT+ and OUT-. The MAX3724 must be DC-coupled to the MAX3748A. See Figures 4 and 5.

For optimum supply-noise rejection, the MAX3725 should be terminated with a matched load. If a singleended output is required, the unused output should be terminated to a 50 Ω resistor to VCC. The MAX3725 does not drive a DC-coupled, 50 Ω grounded load; however, it does drive a compatible 50 Ω CML input.
...
The MAX3724 and the MAX3748A have been optimized to achieve RSSI stability of better than 2.5dB within the 6µA to 500µA range of average input photodiode current.  To  achieve the best accuracy, Maxim recommends receive power calibration to be 6µA at the low end, and 500µA at the high end of the required range.
...
The MAX3724/MAX3725 have high gain, which limits the output when the input signal exceeds 50µAP-P. The MAX3724/MAX3725 operate in a linear range (10% linearity) for inputs not exceeding:
...
Supply voltage noise at the cathode of the photodiode produces a current I = CPD ∆ V/ ∆ t,  which  reduces  the receiver sensitivity (CPD is  the  photodiode capacitance.) The filter resistor of the MAX3724/MAX3725, combined with an external capacitor, can be used to reduce this noise (see the Typical Application Circuit ).
...
For high-current density and reliable operation, the MAX3724/MAX3725 use gold metalization. Connections to the die should be made with gold wire only, using ballbonding techniques. Die thickness is typically 14 mils (0.4mm).
...
|   PAD |   COORDINATES (µm) X |   COORDINATES (µm) Y |
|-------|----------------------|----------------------|
|     1 |                  1.4 |                495.6 |
|     2 |                    0 |                  336 |
|     3 |                    0 |                  224 |
