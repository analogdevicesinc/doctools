...
## MAX11156 EV System Block Diagram
...
- MAX11156	 daughter	 board	 (included	 with	 	 the	 EV system)
...
- 1) Uncompress  the  11156Rxx.ZIP  file  in  a  temporary folder.
...
- 3) Carefully  connect  the  MAXPRECADCMB  (mother board)  with  the  MAX11156  daughter  board  by  aligning the mother board's J1 connector with the daughter board's J1 connector. Gently press them together.
...
Figure 1. MAX11156 Evaluation System Software Main Window
...
The MAX11156 daughter board provides a proven layout for  the  device.  An  external  reference,  level  translators, and  SPI-interface  pads  are  included  on  the  daughter board.
...
Table 2. MAX11156 Daughter Board Jumper Descriptions (JU1-JU16)
...
| JU3      | U1.DIN    | 1-2              | (Interface Mode Selection = Daisy-Chain Mode) Connects the DIN input of the first MAX11156 (U1) to the DOUT output of the second MAX11156 (U3) when JU12 is in the 1-2 position.                                   |
...
| JU3      | U1.DIN    | 1-4              | Connects the DIN input of the MAX11156 (U1) to the EXT_DIN connector.                                                                                                                                              |
...
| JU5      | U1.DOUT   | 1-2*             | Connects the DOUT signal of the MAX11156 (U1) to the first DOUT pin (DOUT) of the FPGA.                                                                                                                            |
| JU5      | U1.DOUT   | 2-3              | Connects the DOUT signal of the MAX11156 (U1) to the EXT_DOUT connector.                                                                                                                                           |
| JU6      | U1.CONVST | 1-2*             | Connects the CONVST signal of the MAX11156 (U1) to the FPGA.                                                                                                                                                       |
| JU6      | U1.CONVST | 2-3              | Connects the CONVST signal of the MAX11156 (U1) to the EXT_CNVST connector.                                                                                                                                        |
| JU7      | U1.REFIO  | 1-2*             | Connects the REFIO of the MAX11156 to the on-board 4.096V reference.                                                                                                                                               |
| JU7      | U1.REFIO  | 2-3              | Connects the REFIO to the EXT_REF connector.                                                                                                                                                                       |
| JU8      | U1.AIN-   | 1-2*             | Connects the AIN- input of the MAX11156 (U1) to ground.                                                                                                                                                            |
| JU8      | U1.AIN-   | 2-3              | Connects the AIN1- connector to the AIN- input of the MAX11156 (U1).                                                                                                                                               |
| JU9      | U1.AIN+   | 1-2*             | Connects the AIN1+ connector to the AIN+ input of the MAX11156 (U1).                                                                                                                                               |
| JU9      | U1.AIN+   | 2-3              | The output of the on-board buffer (U2) connects to the AIN+ input of the MAX11156 (U1).                                                                                                                            |
| JU10     | U3.AIN+   | 1-2*             | Connects the AIN2+ connector to the AIN+ input of the MAX11156 (U3).                                                                                                                                               |
| JU10     | U3.AIN+   | 2-3              | The output of the on-board buffer (U4) connects to the AIN+ input of the MAX11156 (U3).                                                                                                                            |
| JU11     | U3.AIN-   | 1-2*             | Connects the AIN- input of the MAX11156 (U3) to ground.                                                                                                                                                            |
| JU11     | U3.AIN-   | 2-3              | Connects the AIN2- connector to the AIN- input of the MAX11156 (U3).                                                                                                                                               |
| JU12     | U3.DOUT   | 1-2*             | (Interface Mode Selection = CS Mode) (Interface Mode Selection = Daisy-Chain Mode) Connects the DOUT signal of the second MAX11156 (U3) to the DIN pin of the first MAX11156 (U1) when JU3 is in the 1-2 position. |
| JU12     | U3.DOUT   | 2-3              | (Interface Mode Selection = Multichannel CS Mode) Connects the DOUT signal of the MAX11156 (U3) to the DOUT pin of the FPGA.                                                                                       |
...
## Table 2. MAX11156 Daughter Board Jumper Descriptions (JU1-JU16) (continued)

| JUMPER   | SIGNAL     | SHUNT POSITION   | DESCRIPTION                                                                                                                                                                         |
|----------|------------|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| JU13     | U3.CONVST  | 1-2*             | (Interface Mode Selection = CS Mode) (Interface Mode Selection = Daisy-Chain Mode) Connects the CONVST input of the MAX11156 (U3) to the first CONVST signal (CONVST1) of the FPGA. |
| JU13     | U3.CONVST  | 2-3              | (Interface Mode Selection = Multichannel CS Mode) Connects the CONVST input of the MAX11156 (U3) to the second CONVST signal (CONVST2) of the FPGA.                                 |
| JU14     | U1.DOUT.R8 | 1-2*             | (For interface modes with busy indicator.) Connects the pullup resistor to the DOUT output of the MAX11156 (U1).                                                                    |
| JU14     | U1.DOUT.R8 | No shunt         | Disconnects the pullup resistor from the MAX11156 (U1).                                                                                                                             |
...
|------------------|-------|-------------------------------|
| MAX11156DBEVKIT# |     1 | MAX11156 daughter board       |
| MAXPRECADCMB#    |     1 | Serial interface mother board |

## MAX11156 Daughter Board
...
## MAX11156 Daughter Board (continued)
...
| EXT_CONVST, EXT_DIN, EXT_DOUT, EXT_REF, EXT_REFIO, EXT_SCLK |     6 | White multipurpose test points                                      |
...
| U1, U3        |     2 | 16-bit, 500ksps/250ksps, ±5V SARADCs with internal reference (12 TDFN-EP*) Maxim MAX11156ETC+ |
...
| J1                               |     1 | Shrouded 2mm right-angle header (2 x 20) SAMTEC LS2-120-01-S-D-RA1  |
...
## MAXPRECADCMB Mother Board (contined)
...
Indicate that you are using the MAX11156 when contacting these manufacturers.
...
Figure 2a. MAX11156 Daughter Board Schematic (Sheet 1 of 2)

<!-- image -->

Figure 2b. MAX11156 Daughter Board Schematic (Sheet 2 of 2)

<!-- image -->

Figure 3. MAX11156 Daughter Board Component Placement Guide-Component Side

<!-- image -->

Figure 4. MAX11156 Daughter Board PCB Layout-Component Side

<!-- image -->

│

Figure 5. MAX11156 Daughter Board PCB Layout-Ground Layer 2

<!-- image -->

Figure 6. MAX11156 Daughter Board PCB Layout-Power Layer 3

<!-- image -->

Figure 7. MAX11156 Daughter Board PCB Layout-Solder Side

<!-- image -->

Figure 8. MAX11156 Daughter Board Component Placement Guide-Solder Side
