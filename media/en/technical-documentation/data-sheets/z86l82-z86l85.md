...
## Z86L82/Z86L85 28-Pin Low-Voltage IR Microcontrollers
...
Table 1 lists some of the features of the Z86L82/85 microcontrollers.

Table 1. Z86L82/85 Features

| Device                 | ROM (KB)               |   RAM* (Bytes) |   I/O Lines | Voltage Range   |
|------------------------|------------------------|----------------|-------------|-----------------|
| Z86L82                 | 4                      |            237 |          23 | 2.0 V to 3.6 V  |
| Z86L85                 | 8                      |            237 |          23 | 2.0 V to 3.6 V  |
...
- The mask option pull-up transistor has a typical equivalent Note:
- resistance of 200 K  ±50% at V CC =3 V and 450 K  VCC =2 V.
...
The Z86L82/85 architecture is based on Maxim's 8-bit microcontroller core featuring an Expanded Register File to allow access to register-mapped peripherals, I/O circuits, and powerful counter/timer circuitry. The Z8 offers a flexible I/O scheme, an efficient register and address-space structure, and a number of ancillary features that are useful in many consumer, automotive, computer peripheral, and battery-operated hand-held applications.
...
The Z86L82/85 offers a new intelligent counter/timer architecture with 8-bit and 16-bit counter/timers (Figure 1). Also included are a large number of user-selectable modes and two on-board comparators to process analog signals with separate reference voltages (see Figure 9 on page 18).

±50% at All signals with an overline, ' ', are active Low. For example, B/W, in which WORD is active Low, and B/W, in which BYTE is active Low. Note:
...
Table 4 lists the absolute maximum ratings for the Z86L82/85 microcontrollers.
...
Table 5 lists the capacitance for the Z86L82/85 microcontrollers.
...
- Internal pull-ups are disabled on any given pin or group of port Note:
...
The Z86L82/85 incorporates special functions to enhance the Z8's functionality in consumer and battery-operated applications.

## Program Memory

The Z86L82/85 family addresses 4/8 KB of internal program memory. The first 12 bytes are reserved for interrupt vectors. These locations contain the five 16-bit vectors that correspond to the five available interrupts.

## RAM

<!-- image -->

The Z86L82/85 device has 237 bytes of RAM that make up the register file.
...
The upper nibble of the register pointer (Figure 12 on page 22) selects which working register group is accessed of 16 bytes in the register file, out of the possible 256. The lower nibble selects the expanded register file bank and, in the case of the Z86L8X family, banks 0, F, and D are implemented. A 0h in the lower nibble allows the normal register file (bank 0) to be addressed, but any other value from 1h to Fh exchanges the lower 16 registers to an expanded register bank. For example, for the Z86L8X (see Figure 11):
...
The Z86L82/85 internal register file is used for the stack. An 8-bit Stack Pointer (R255) is used for the internal stack that resides in the general-purpose registers (R4-R239). SPH is used as a general-purpose register only when using internal stacks.
...
A 1 in this bit position indicates that a High level on any one of the recovery sources wakes the Z86L8X from STOP Mode. A 0 indicates Low level recovery. The default is 0 on POR.
...
Z86L8208PSC Z86L8508PSC

## 28-Pin SOIC

Z86L8208SSC Z86L8508SSC

For the die form, please contact Maxim. Note:
