...
This document explains how the Munich (USB2PMB2#) adapter board receives commands from a laptop through the  USB  to  create  an  I 2 C/SPI  interface.  Hardware design files as well as results on interfacing USB2PMB2# with MAX11300PMB1, Santa Fe (MAXREFDES5#) , Campbell (MAXREFDES4#) , Fresno (MAXREFDES11#) , and Corona (MAXREFDES12#) are provided.

Portability  has  become  another  word  for  convenience. These days, almost all of us carry laptops, and this advantage  can  be  used  to  showcase  parts  to  the  customers offering solutions that can cater to their needs. To have that convenience, we need to have a board that interfaces with  our  laptops  to  the  signal  chain  electronics  that  are present on many of the industrial, medical, and consumer applications.  This  adapter  board  serves  this  purpose  of being  a  bridge  between  your  laptop  and  your  I 2 C/SPIenabled devices.

The  Maxim  USB2PMB2#  board  requires  custom  software  and  can  be  ordered  together  as  an  evaluation system  (EV  system).  The  Munich  design  integrates  a low-power,  1.2A,  PWM  step-down  DC-DC  converter (MAX1556);  a  dual,  high-speed,  USB-to-multipurpose UART/FIFO  IC  (FT2232HL),  high-speed  USB  switches, and  a  4K  MICROWIRE ® -compatible  serial  EEPROM. This  Munich  adapter  board  is  used  to  enable  USBto-I 2 C/SPI  interface  for  any  Pmod™-compatible  plugin  peripheral  modules  such  as  the  Maxim  Campbell, Santa Fe, Fresno, and Corona reference designs.

## USB2PMB2# Adapter Board

Ordering information for EV systems is included in the EV kits' corresponding data sheet. An EV system is an EV kit combined with an interface board such as a USB2PMB2# and custom software. Refer to the appropriate EV kit data sheet for quick start and detailed operating instructions. The  USB2PMB2#  has  been  tested  on  Windows ® 7, Windows 8, and Windows XP ® .
...
Figure 1. Munich USB2PMB2# Board
...
Figure  3  shows  the  pin  configuration  for  the  SPIcompatible connector at the USB2PMB2# board. Figure 4 depicts the pin configuration of connector found

## USB2PMB2# Adapter Board

Figure 3. X2: Pmod I 2 C/SPI Connector Pin Configuration
...
in Santa Fe, Campbell, Fresno, and Corona. The connector  type  is  female  on  the  USB2PMB2#  board  and  male type on the drivers.
...
- Munich (USB2PMB2#)  Board
- A-to-Mini-B USB cable
- USB2PMB2# custom software
...
- 1)  Go  to www.maximintegrated.com/evkitsoftware to download  the  most  recent  version  of  the  Munich board software, MUNICH  GUI . Save the software  to  a  temporary  folder  and  decompress  the Munich\_GUISetupV2.04.zip file.
...
Figure 16. MAX11259PMB GUI

<!-- image -->

## MAX11259PMB GUI

The USB2PMB2 adapter board can also be used to evaluate the performance of the MAX11259 24-bit, 6-channel, 64ksps integrated PGA delta-sigma ADC with I 2 C interface. For detailed information, refer to the MAX11259PMB data sheet.
