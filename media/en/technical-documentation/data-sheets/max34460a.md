...
| Host Sends Improperly Set Read Bit in the Target Address Byte . . . . . . . . . . . . . . . . . . . . . . . . | . 22                |
...
| MFR_VOUT_MIN (D7h) . . . . . . . . . . . . . . . . . . .           | .49                             |
| MFR_FW_SERIAL (E0h) . . . . . . . . . . . . . . . . . .            | .49                             |
| MFR_TEMPERATURE_AVG (E3h) . . . . . . . . . .                      | .49                             |
| MFR_CHANNEL_NOMINAL (EAh) . . . . . . . . . .                      | .49                             |
...
| RS Input Leakage                        | IL RS                            |                                  |        | ±0.25 |       | µA           |
...
| TIMING OPERATING CHARACTERISTICS        | TIMING OPERATING CHARACTERISTICS | TIMING OPERATING CHARACTERISTICS |        |       |       |              |
...
## PMBus 12-Chanel V oltage Monitr and Sequncr
...
* All pins except V DD , EP/V SS , ALERT , A1/PG, and REG18 are high impedance during device power-up and reset.
...
| 8Dh 98h | READ_TEMPERATURE_1  | Read word Read byte | - R         | R R          | -          | 2 1            | N/N                             | 0000h 11h                |
|         | PMBUS_REVISION      |                     |             |              | R          |                | FIXED/N                         |                          |

## PMBus 12-Channel Voltage Monitor and Sequencer

temperature  sensors  is  conducted  through  a  dedicated I 2 C/SMBus interface.
...
| CODE   | COMMAND NAME           | TYPE       | PAGE 0-11   | PAGE 13-17   | PAGE 255   | NO. OF BYTES   | FLASH STORED/ LOCKED   | DEFAULT VALUE (NOTE 2)   |
|--------|------------------------|------------|-------------|--------------|------------|----------------|------------------------|--------------------------|
| CODE   | COMMAND NAME           | TYPE       | (NOTE 1)    | (NOTE 1)     | (NOTE 1)   | NO. OF BYTES   | FLASH STORED/ LOCKED   | DEFAULT VALUE (NOTE 2)   |
...
tored  to  make  sure  that  the  PSEN0  supply  crosses  the undervoltage-fault  limit  (as  configured  in  VOUT\_UV\_ FAULT\_LIMIT)  within  a  programmable  time  limit  (as configured  in  TON\_MAX\_FAULT\_LIMIT).  When  RS0 crosses  the  undervoltage-fault  limit,  timeslot  1  begins. PSEN2  and  PSEN5  have  been  assigned  to  timeslot  1 and each has their own unique TON\_DELAY and TON\_ MAX\_FAULT\_LIMIT  values.  Since  two  power  supplies have  been  assigned  to  timeslot  1,  the  last  power  supply  to  cross  its  associated  undervoltage-fault-limit  level
...
In  both  dependent  and  independent  modes,  the  WDO pin  can  also  be  configured  to  act  as  a  manual-reset (MR)  input.  When  the  WD\_WDO\_MR  bit  is  set  in MFR\_WATCHDOG\_CONFIG,  the WDO pin  becomes a  digital  input/output  pin  that  detects  a  falling  edge, debounces the falling edge for a 200ms period, and then once  the  manual  reset  has  been  qualified,  the  device asserts  the WDO output  low  for  the  time  configured  in MFR\_PG\_DELAY before releasing the pin and restarting the watchdog function. To prevent glitches from occurring in the reset signal, the device seizes the reset signal as soon as it qualifies the manual reset and holds the WDO output low for the MFR\_PG\_DELAY time. The MFR\_PG\_ DELAY time should be configured for a time longer than the manual-reset pulse length. When the time configured in  MFR\_PG\_DELAY  expires,  the  device  releases  the reset signal and restarts the watchdog as long as the pin is high. If the pin is low when the WDO pin is released, the device waits for the pin to go high (debounced by 20ms) before restarting the watchdog function. See Figure 4.

<!-- image -->

## MAX34460A

## PMBus 12-Chanel Voltage Monitor and Sequencr
...
Active margining begins once power good has been exceeded for all channels.
...
In applications where the measured power-supply voltage  is  not  equal  to  the  voltage  at  the  ADC  input, VOUT\_SCALE\_MONITOR  is  used.  For  example,  if  the ADC input expects a 1.8V input for a 12V output, VOUT\_ SCALE\_MONITOR  =  1.8V/12V  =  0.15.  In  applications where  the  power-supply  output  voltage  is  greater  than the  device's  2.048V  input  range,  the  output  voltage  of the  power  supply  is  sensed  through  a  resistive  voltagedivider.  The  resistive  voltage-divider  reduces  or  scales
...
| Undervoltage                                                              | • Power supply enabled (TON_MAX_FAULT_LIMIT ≠ 8000h to FFFFh) • PSENn output is active • Channel's VOUT must have exceeded VOUT_UV_FAULT during channel power-up Stop is off. | monitoring while the power supply |
...
## PMBus 12-Chanel Voltage Monitor and Sequncr
...
|     6 | SYS_OFF     | Set when any of the power supplies are sequenced off (logical OR of all the OFF bits in STATUS_ MFR_SPECIFC).                                       |
...
Note 2: The FAULTn status bits are set even if the FAULTn pins are configured in MFR\_MODE to ignore the FAULT pin. The FAULT2\_INPUT status bit only functions if the device is in dual-sequencing mode.
...
|       |                  | These bits select the postADC conversion averaging:                                                                                                                                                                                                                                                                                                                                                                                                                                                        | These bits select the postADC conversion averaging:                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 5     | GROUP (dual-sequence mode only) | This bit is used in dual-sequencing mode to determine the sequencing channel assignment. This bit is ignored if the dual-sequencing mode is disabled (DUAL_ SEQ bit in MFR_MODE is cleared). 0 = Channel is assigned to the primary sequencing group. 1 = Channel is assigned to the secondary sequencing group. | This bit is used in dual-sequencing mode to determine the sequencing channel assignment. This bit is ignored if the dual-sequencing mode is disabled (DUAL_ SEQ bit in MFR_MODE is cleared). 0 = Channel is assigned to the primary sequencing group. 1 = Channel is assigned to the secondary sequencing group. | This bit is used in dual-sequencing mode to determine the sequencing channel assignment. This bit is ignored if the dual-sequencing mode is disabled (DUAL_ SEQ bit in MFR_MODE is cleared). 0 = Channel is assigned to the primary sequencing group. 1 = Channel is assigned to the secondary sequencing group. |
...
When  the  device  is  configured  to  operate  in  dualsequencing mode (the DUAL\_SEQ bit in MFR\_MODE is
...
DAC 'RFS' = (7.75)/(IFB x Margining Range) where IFB is the feedback node current. Example:

IFB = 500µA, margining range = ±15% DAC 'RFS' value = (7.75)/(500µA x 15%) = 103kΩ Note: 40kΩ &lt; RFS &lt; 160kΩ
...
| PAGE   | MAX34460A TEMP SENSOR   | DS75LV ADDRESS PIN CONFIGURATION   | DS75LV ADDRESS PIN CONFIGURATION   | DS75LV ADDRESS PIN CONFIGURATION   |
|--------|-------------------------|------------------------------------|------------------------------------|------------------------------------|
| PAGE   | MAX34460A TEMP SENSOR   | A2                                 | A1                                 | A0                                 |
...
The  MFR\_WATCHDOG\_CONFIG  command  is  used  to configure  the  external  watchdog  function.  The  MFR\_ WATCHDOG\_CONFIG command is described in Table 40.
...
| 11:8  | WD_TIMEOUT[3:0] | 0000                                                                                                                              | 5ms                                                                                                                               | 1000                                                                                                                              |
| 11:8  | WD_TIMEOUT[3:0] | 0001                                                                                                                              | 10ms                                                                                                                              | 1s 1001 2s                                                                                                                        |
...
| 11:8  | WD_TIMEOUT[3:0] | 0111                                                                                                                              | 640ms                                                                                                                             | 1111 128s                                                                                                                         |
...
|                 1 | 4/24            | Updated Pin Description table; updated MAX34460AA00 to MAX34460A; updated master/slave to controller/target to match the current I 2C standard | All             |
