...
Maximum Gain, Gain Range, and OutputReferred Noise Optimized for Interfacing with 10-Bit ADCs:

Maximum Gain of 39.5dB Total Gain Range of 50dB 60nV/ √ Hz Ultra-Low Output-Referred Noise at 5MHz
...
- Serial-Programmable LO Phase Generator for 4, 8, 16 LO Quadrature Phase Resolution Optional Individual Channel 4 x fLO LO Input Drive Capability
...
| R3, R4, R11, R12, R13, R19, R31, R32, R35, R42, R53, R54, R61, R62, R84, R85                                                               |    16 | 28 ±1% resistors (0603) Any                                                       |
...
| R8, R9, R14, R15, R20, R23-R26, R40, R41, R45, R46, R50, R57, R58                                                                          |    16 | 475 ±1% resistors (0603) Any                                                      |
| R38                                                                                                                                        |     1 | 100 ±1% resistor (0603) Any                                                       |

| DESIGNATION                                                                                                      |   QTY | DESCRIPTION                  |
|------------------------------------------------------------------------------------------------------------------|-------|------------------------------|
| R67-R82, R94, R95, R99- R102, R104, R105, R114- R129, R143- R146, R150- R163, R172, R173, R175, R177, R186, R187 |    64 | 0 resistors (0603) Any       |
| R87                                                                                                              |     1 | 7.5k ±1% resistor (0603) Any |
| R88, R89                                                                                                         |     2 | 10k ±5% resistors (0603) Any |
| R93, R130                                                                                                        |     2 | 3k ±5% resistors (0603) Any  |
| R103, R112                                                                                                       |     2 | 2k ±5% resistors (0603) Any  |
| R106-R110, R147, R149                                                                                            |     7 | 200 ±1% resistors (0603) Any |
| R111                                                                                                             |     1 | 124 ±1% resistor (0603) Any  |
| R113                                                                                                             |     1 | 887 ±1% resistor (0603) Any  |
...
| DESIGNATION                  |   QTY | DESCRIPTION                                               |
|------------------------------|-------|-----------------------------------------------------------|
| R131-R142                    |    12 | 360 ±0.1% metal-film resistors (0805) Digi-Key P360ZCT-ND |
| R178                         |     1 | 20k ±5% resistor (0603) Any                               |
| R179, R180, R181, R184, R185 |     5 | 5.11k ±1% resistors (0603) Any                            |
| R182                         |     1 | 1.5k ±1% resistor (0603) Any                              |
| R183                         |     1 | 1k ±1% resistor (0603) Any                                |
...
The MAX2036 EV kit is fully assembled and factory tested. Follow the instructions in the Connections and Setup section for proper device evaluation.

## Test Equipment Required

This section lists  the  recommended test equipment to verify the operations of the MAX2036. It is intended as a guide only, and substitutions may be possible.
...
This section provides a step-by-step guide to testing the basic functionality of the MAX2036 EV kit. As a general  precaution to prevent damaging the outputs, do not turn on DC power or signal generators until all connections are completed.
...
The variable-gain amplifier (VGA) section of the MAX2036 has a low noise input section, VGA section, and output driver function. A total of 8 channels of complete variable-gain functionality are provided in a single chip. The MAX2036 EV kit allows the device to be driven either in a broadside manner, with all channels driven simultaneously from a single source, or with individual drive to each channel. The gain-control interface allows a simple 0 to 2V input signal to control the gain of the device for all 8 channels. The variable-gain outputs are provided on eight sets of differential output coax connectors, for connection to ADCs or measurement equipment.

Provisions are made to adjust the various ancillary functions of the device, such as programmable clipping, etc.

The CW section of the MAX2036 has an input quadrature  mixer  section  and  a  baseband signal-combining section. The input is common with the VGA input, coupled into the CW section through a noise-limiting lowpass filter. The output from the summed 8 channels is a quadrature baseband differential current-source output. This current output is terminated by fixed resistors to produce a voltage output for ease of measurement.
...
| CONTROL BITS   | CONTROL BITS   |      |                    |                 |                          |                              |                                        |                              | NO. OF                    |
|----------------|----------------|------|--------------------|-----------------|--------------------------|------------------------------|----------------------------------------|------------------------------|---------------------------|
| CW_M1          | CW_M2          | MODE | LO INPUT FREQUENCY | CLOCK INTERFACE | PHASE RESOLUTION         | NO. OF CLOCK INPUTS PER CHIP | PROGRAM BY SERIAL SHIFT REGISTER (SSR) | NO. OF USEFUL BITS IN SSR/CH | DON'T CARE BITS IN SSR/CH |
...
Tables 2-5 illustrate the bit patterns that must be programmed into the MAX2036 device to generate the desired phase angle for each individual channel. These bit patterns are programmed into each channel through the serial  data  port,  as  described  in  the Programming the CWD Beamformer section.
...
Figure 2 illustrates the serial programming of the 8 individual channels through the serial data port (Note that the serial data can be daisy chained from one port to another, allowing a single data line to be used to program multiple chips in the system).

Figure 3 illustrates the overall block diagram of the CWD beamformer section of the MAX2036. The 8-input RF signals are applied to programmable lowpass filters. These filters  remove aliased noise signals from the preceding stage. The RF signals are then mixed to baseband by 8 pairs of quadrature mixers. The quadrature LO signals
...
The  MAX2036  EV  kit  can  be  ordered  online  at www.maxim-ic.com . The CMAXQUSB command module uses a PC's USB port to allow programming through
...
## Using a Pulse Generator to Test the Phase Rotation Beamforming Function of the MAX2036

The phase rotator  beamforming function  of  the MAX2036 requires a phase synchronous RF input and LO input to the EV kit board. The TEST\_MODE output signal from the MAX2036 can be used for the RF input function. This signal will be frequency divided on-chip to  match the correct RF input frequency for the programmed mode of operation. It will be phase synchronous with the internally generated LO signals for each individual channel, always being at a phase of 0°.
...
operated in a gated continuous mode of operation. It will always start in a consistent phase state when gated on by the LOAD line signal, which latches the serially programmed phase information into the MAX2036 beamformer.
...
Figure 10a. MAX2036 EV Kit Schematic (Sheet 1 of 3)
...
Figure 10b. MAX2036 EV Kit Schematic (Sheet 2 of 3)
...
Figure 10c. MAX2036 EV Kit Schematic (Sheet 3 of 3)
...
Figure 11. MAX2036 EV Kit PCB Layout-Top Silkscreen
...
Figure 12. MAX2036 EV Kit PCB Layout-Top Soldermask
...
Figure 13. MAX2036 EV Kit PCB Layout-Top Layer (Metal)
...
Figure 14. MAX2036 EV Kit PCB Layout-Inner Layer 2 (GND)
...
Figure 15. MAX2036 EV Kit PCB Layout-Inner Layer 3 (Routes)
...
Figure 16. MAX2036 EV Kit PCB Layout-Bottom Silkscreen
...
Figure 17. MAX2036 EV Kit PCB Layout-Bottom Soldermask
...
Figure 18. MAX2036 EV Kit PCB Layout-Bottom Layer (Metal)
...
Figure 19. MAX2036 EV Kit PCB Layout-Drill and Mechanical
