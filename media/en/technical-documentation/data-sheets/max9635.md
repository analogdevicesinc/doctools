...
The IC is designed to operate from a 1.7V to 3.6V supply  voltage  range  and  consumes  only  0.65µA  in  full operation. It is available in a small, 2mm x 2mm x 0.6mm OTDFN package.
...
|-------------|---------------|--------------------|
| MAX9635EDT+ | 6 OTDFN-EP*   | -40 N C to +85 N C |
...
6 OTDFN (derate 11.9mW/ N C above +70 N
...
| Power-Supply Current               | ICC                         | T A = +25 N C, 90 lux, I 2 C inputs inactive           |                             | 0.65                        | 1.2                         | F A                         |
...
| REGISTER                  | BIT   | BIT    | BIT   | BIT   | BIT   | BIT      | BIT      | BIT      | REGISTER ADDRESS   | POWER-ON RESET STATE   | R/W   |
|---------------------------|-------|--------|-------|-------|-------|----------|----------|----------|--------------------|------------------------|-------|
...
If the user wants to read both the Lux High-Byte register 0x03 and Lux Low-Byte register 0x04, then the master should not send a STOP command between the reads of the two registers. Instead a REAPEATED START command should be used. This ensures accurate data is obtained from the I 2 C registers (by disabling internal updates during the read process).
...
| APPLICATION CONDITIONS   | APPLICATION CONDITIONS   | APPLICATION CONDITIONS   | APPLICATION CONDITIONS   | RECOMMENDED SETTINGS FOR CONFIGURATION REGISTER (0x02)   | RECOMMENDED SETTINGS FOR CONFIGURATION REGISTER (0x02)   | RANGE OF EXPONENTS FOR UPPER AND LOWER THRESHOLD REGISTERS (0x05 AND 0x06)   | RANGE OF EXPONENTS FOR UPPER AND LOWER THRESHOLD REGISTERS (0x05 AND 0x06)   |
|--------------------------|--------------------------|--------------------------|--------------------------|----------------------------------------------------------|----------------------------------------------------------|------------------------------------------------------------------------------|------------------------------------------------------------------------------|
