...
The MAX3875 is designed for both section-regenerator and terminal-receiver applications in OC-48/STM-16 transmission systems. Its jitter performance exceeds all of the SONET/SDH specifications.
...
|------------|----------------|---------------|
| MAX3875EHJ | -40°C to +85°C | 32 TQFP       |
| MAX3875E/D | -40°C to +85°C | Dice*         |
...
Maxim Integrated Products 12-91
...
| PECL Output Voltage (SDO+, SDO-, SCLKO+, SCLKO-) .......................(V CC + 0.5V)                                   |
| PECL Output Current, (SDO+, SDO-, SCLKO+, SCLKO-).....56mA                                                              |
| Voltage at LOL , SIS, PHADJ+, PHADJ-, FIL+, FIL-.................................................-0.5V to (V CC + 0.5V) |
...
| Differential Input Voltage (SDI±, SLBI±) | V ID     | Figure 1                          | 50           |       | 800        | mVp-p   |
| Single-Ended Input Voltage (SDI±, SLBI±) | V IS     |                                   | V CC - 0.4   |       | V CC + 0.2 | V       |
| Input Termination to V CC (SDI±, SLBI±)  | R IN     |                                   | 45           | 45    | 45         | Ω       |
...
| Jitter Tolerance                       |          | f = 70kHz                  |  1.91 |   3.6 |       | UIp-p   |
| Jitter Tolerance                       |          | f = 100kHz                 |  1.76 |  2.75 |       | UIp-p   |
| Jitter Tolerance                       |          | f = 1MHz                   |  0.41 |  0.67 |       | UIp-p   |
| Jitter Tolerance                       |          | f = 10MHz (Note 3)         |  0.21 |  0.45 |       | UIp-p   |
| Jitter Generation                      | J GEN    | Jitter BW = 12kHz to 20MHz |       | 0.003 | 0.006 | UI RMS  |
| Jitter Generation                      |          |                            |       | 0.026 | 0.056 | UIp-p   |
...
The MAX3875 consists of a fully integrated phaselocked loop (PLL), input amplifier, data retiming block, and PECL output buffer (Figure 3). The PLL consists of a  phase/frequency detector (PFD), a loop filter, and a voltage-controlled oscillator (VCO).
...
800mVp-p. The bit error rate is better than 1 · 10 -10 for input signals as small as 10mVp-p, although the jitter tolerance performance will be degraded. For interfacing with PECL signal levels, see Applications Information .
...
A loss-of-lock ( LOL )  monitor is incorporated in the MAX3875 frequency detector. A loss-of-lock condition is signaled immediately with a TTL low. When the PLL is frequency locked, LOL switches to TTL high in approximately 800ns.

Note that the LOL monitor is only valid when a data stream is present on the inputs to the MAX3875. As a result, LOL does not detect a loss-of-power condition resulting from a loss of the incoming signal.
...
When the received data amplitude is higher than 50mVp-p, the MAX3875 provides a typical jitter tolerance of 0.45UI at jitter frequencies greater than 10MHz. The SDH/SONET jitter tolerance specification is 0.15UI, leaving a jitter allowance of 0.3UI for receiver preamplifier and postamplifier design.
...
The MAX3875 has a low phase and frequency drift in the absence of data transitions. As a result, long runs of consecutive zeros and ones can be tolerated while maintaining a BER of 1 · 10 -10 .  The  CID  tolerance  is tested using a 2 13  - 1 PRBS, substituting a long run of zeros to simulate the worst case. A CID tolerance of 2000 bits is typical.
