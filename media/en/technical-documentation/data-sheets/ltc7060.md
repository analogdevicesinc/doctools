...
Its powerful 0.8Ω pull-down and 1.5Ω pull-up MOSFET drivers allows the use of large gate capacitance high voltage MOSFETs. Additional features include UVLO, ThreeState  PWM  input,  adjustable  turn-on/-off  delays  and shoot-through protection.
...
PWM Pin Thresholds vs Temperature vs Temperature PWM Pin Thresholds
...
VCC  Undervoltage Lockout Thresholds vs Temperature Thresholds vs Temperature VCC  Undervoltage Lockout
...
VCC  Overvoltage Lockout Thresholds vs Temperature Thresholds vs Temperature VCC  Overvoltage Lockout
...
Propagation Delay vs Temperature vs Temperature Propagation Delay
...
TG/BG Pull-Up and Pull-Down Resistance vs Temperature Resistance vs Temperature TG/BG Pull-Up and Pull-Down
...
A simplified version of the LTC7060's output stage is shown in Figure 2. The BG and TG design are symmetric and they both have floating gate driver outputs. The pullup device is a PMOS with a typical 1.5Ω R DS(ON)  and the pull-down device is a NMOS with a typical 0.8Ω R DS(ON) . The wide driver supply voltage ranging from 4V to 14V enables the driving of different power MOSFETs, such as logic level or higher threshold MOSFETs. However , the LTC7060 is optimized for higher threshold MOSFETs (e.g. BST-SW = 10V and BGV CC -BGRTN = 10V). The driver
...
The charge to turn on the external MOSFET is referred to gate charge, Q G , and is typically specified in the external MOSFET data sheet. The boost capacitor , C B , needs to have at least 10 times the gate capacitance to turn on the external MOSFET fully. Gate charge can range from 5nC to hundreds of nC and is influenced by the gate drive level and type of external MOSFET used. For most applications, a capacitor value of 0.1uF for C B  will be sufficient. However, if multiple MOSFETs are paralleled and driven by the LTC7060, C B  capacitance needs to be increased correspondingly.

An external supply, typically V CC  connected through a Schottky diode, is required to keep the C B  charged. The LTC7060 does not charge the C B  and always discharges the C B . When the BG/TG is high, the total current from BGVCC/BST to BGRTN/SW and SGND is typically 100µA; when the BG/TG is low, the total current from BGV CC /BST is typically 8µA.

## POWER DISSIPATION

To ensure proper operation and long-term reliability, the LTC7060 must not operate beyond its maximum temperature rating. Package junction temperature can be calculated by:
...
θ JA  = junction-to-ambient thermal resistance Power dissipation consists of standby, switching and capacitive load power losses:
...
The LTC7060 consumes very little quiescent current. The DC power loss at V CC  = 10V is only (10V)(0.4mA) = 4mW.
...
The  LTC7060  requires  proper  bypassing  on  the  V CC , VBST-SW ,  and  V BGVCC-BGRTN   supplies  due  to  its  high speed switching (nanoseconds) and large AC currents (amperes).  Careless  component  placement  and  PCB trace routing may cause excessive ringing and under/ overshoot.
...
4-Phase 1.2V/120A Converter with the LTC7060 and MOSFETs Using the LTC7851, f SW  = 400kHz
...
|-------|--------|-----------------------------------------------------------------------|---------------|
| A     | 04/20  | Updated Order Information. R DT changed from 64.9kΩ to 100kΩ in I VCC | 2 3           |
