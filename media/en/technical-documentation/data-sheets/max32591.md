...
The secure microcontroller includes multiple communication  interfaces.  One  USB  host  controller  and  one  USB device  controller  with  their  respective  USB  transceiver, two smart card controllers, five SPI ports, three UARTs, an Ethernet 10/100 MAC with FIFO, and an I 2 C bus are also provided. The three on-chip timers also support PWM output generation for direct control of external devices. An integrated secure keypad provides an integrated solution for mobile POS terminals. Additionally, a 2-channel, 10-bit ADC is provided for printer support and general use.
