...
| TABLE OF CONTENTS                                                                                                                                                   |          |
| REVISION HISTORY                                                                                                                                                    |          |
| 6/2026-Rev. A to Rev. B Changed AD4055 to AD4062 (Throughout).....................................................................................................1 |          |
...
| Analog Input Leakage Current              | IN+, IN-                                           |        | 6 3.4   |           | nA      |
| Sampling Capacitance (C IN )              | IN+, IN-                                           |        |         |           | pF      |
| Track Phase                               |                                                    |        | 5.4     |           | pF      |
| Hold Phase                                |                                                    |        | 2.0     |           | pF      |
...
The AD4062 operates in two phases, the acquisition phase and the conversion phase. In the acquisition phase, the internal track-andhold circuitry is connected to each input pin (IN+ and IN-) and acquires the voltage on each pin independently. The AD4062 remains in the acquisition phase until the CONV\_READ or CONV\_TRIG-
...
| Burst Averaging Mode | CONV_READ CONV_TRIGGER | 0x52 0x58                           |
...
|--------|------------|----------------------------------------------------------------------------------------------------------------------|---------|----------|
| 7      | RESERVED   | Reserved.                                                                                                            | 0x0     | R        |
| [6:0]  | TGT_ADDR   | Assigned Target Address. Contains the Target address assigned to the AD4062 during dynamic address assignment (DAA). | 0x0     | R        |
...
| [6:0]  | GRP_ADDR         | Assigned Group Address. Contains the Group address assigned to the AD4062 by the SETGRPA CCC command. If GRP_NOT_ASSIGNED is 0, GRP_ADDR contains the assigned group address. Otherwise, GRP_ADDR reads back all 1's.                                                                                                                         | 0x7F    | R        |
...
The AD4062 ADC core performs conversion(s) following I3C reads while the ADDR\_PTR is pointing to the CONV\_READ register address.
...
The AD4062 ADC core performs conversion(s) following I3C writes which set the ADDR\_PTR to the CONV\_TRIGGER register address. By using I3C Group addressing, this register can be used to synchronize conversions across multiple AD4062 devices on the same I3C bus.
...
Figure 133. 14-Lead Lead Frame Chip Scale Package [LFCSP] 2 mm × 2.6 mm Body and 0.75 mm Packaging Height (CP-14-7)
