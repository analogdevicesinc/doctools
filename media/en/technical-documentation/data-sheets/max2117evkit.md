...
The MAX2117 evaluation kit (EV kit) simplifies the testing and evaluation of the device direct-conversion tuner. The evaluation kit is fully assembled and tested at the factory. Standard 50Ω SMA and BNC connectors are included on the EV kit for the inputs and outputs to allow quick and easy evaluation on the test bench.

This  document  provides  a  list  of  equipment  required  to evaluate the device, a straightforward test procedure to verify functionality, a description of the EV kit circuit, the circuit schematic, a component list for the kit, and artwork for each layer of the printed-circuit board (PCB).
...
| BB_IN, BB_IP, BB_QN, BB_ QP, CP_OUT, J12, J13, J17, REF_O/P, VGC |    10 | PC mini red test points Keystone 5000                                        |
...
| C10, C11, C16                                                    |     3 | 0.047µF ±10% ceramic capacitors (0603) Murata GRM188R71C473K                 |
...
| DESIGNATION                                                          |   QTY | DESCRIPTION                                                                   |
|----------------------------------------------------------------------|-------|-------------------------------------------------------------------------------|
| C15                                                                  |     1 | 0.68µF ±10% ceramic capacitor (0603) Murata GRM188R61A684K                    |
...
| JP_VCC, VCC_ BB, VCC_DIG, VCC_LO, VCC_RF1, VCC_RF2, VCC_SYN, VCC_VCO |     0 | Not installed, 2-pin (1 x 2) inline headers, 0.01in centers Sullins PEC36SAAN |
...
| DESIGNATION                         |   QTY | DESCRIPTION                                                                |
|-------------------------------------|-------|----------------------------------------------------------------------------|
| L1                                  |     1 | 68pF ±5% capacitor (0603) Murata GRM1885C1H680J                            |
...
| R46, R47                            |     2 | 2.7kΩ ±5% resistors (0603); use lead-free parts only                       |
...
Note: Indicate that you are using the MAX2117 when contacting these component suppliers.
...
| U1            |     1 | DVBS tuner (28 TQFN-EP**) Maxim MAX2117CTI+                                                                                                |
...
| U5            |     0 | Not installed, I/O comparator MAX985                                                                                                       |
...
- RF signal generator capable of delivering at least 0dBm of output power at frequencies up to 1GHz
- RF spectrum analyzer capable of covering the operating frequency range of the device
- PC laptop or tablet with Microsoft Windows XP ® , Windows ® 7, 8 OS and a spare USB port
...
The  EV  kit  includes  on-board  matching  circuitry  at  the MAX2117 RF input to convert the 50Ω source to a 75Ω input.  Note  that  the  input  power  to  the  device  must be  adjusted  to  account  for  the  -6dB  power  loss  of  the matching resistor network.
...
- 3) With its output disabled, set the DC power supply to 3.3V. Connect the power supply to the VCC (through

Windows and Windows XP are registered trademarks and registered service marks of Microsoft Corporation.

## Evaluates: MAX2117

an ammeter if desired) and GND terminals on the EV kit. If available, set the current limit to 200mA.
...
- 8) Install  and  run  the  IC's  control  software.  Software is  available  for  download  on  the  Maxim  website  at www.maximintegrated.com/evkitsoftware .
...
Figure 2. MAX2117 EV Kit PCB Layout-Component Placement Guide

<!-- image -->

Evaluates: MAX2117

Figure 3. MAX2117 EV Kit PCB Layout-Top

<!-- image -->

Evaluates: MAX2117

Figure 4. MAX2117 EV Kit PCB Layout-Bottom

<!-- image -->

Evaluates: MAX2117

Figure 5. MAX2117 EV Kit PCB Layout-Top Soldermask

<!-- image -->

Evaluates: MAX2117

Figure 6. MAX2117 EV Kit PCB Layout-Bottom Soldermask
