...
## DS1554 256k, Nonvolatile, Y2K-Compliant
...
A temperature compensated comparator circuit monitors the level of VCC. When VCC falls to the powerfail trip point, the RST signal (open drain) is pulled low. When V CC returns to nominal levels, the RST signal  continues  to  be  pulled  low  for  a  period  of  40  ms  to  200  ms.  The  power-on  reset  function  is independent of the RTC oscillator and thus is operational whether or not the oscillator is enabled.
...
| 7FFFh     | 10 Year | 10 Year    | 10 Year    | 10 Year    | Year    | Year    | Year    | Year    | Year           | 00-99 |
| 7FFEh     | X       | X          | X          | 10 Month   | Month   | Month   | Month   | Month   | Month          | 01-12 |
| 7FFDh     | X       | X          | 10 Date    | 10 Date    | Date    | Date    | Date    | Date    | Date           | 01-31 |
| 7FFCh     | X       | FT         | X          | X          | X       | Day     | Day     | Day     | Day            | 01-07 |
| 7FFBh     | X       | X          | 10 Hour    | 10 Hour    | Hour    | Hour    | Hour    | Hour    | Hour           | 00-23 |
| 7FFAh     | X       | 10 Minutes | 10 Minutes | 10 Minutes | Minutes | Minutes | Minutes | Minutes | Minutes        | 00-59 |
| 7FF9h     | OSC     | 10 Seconds | 10 Seconds | 10 Seconds | Seconds | Seconds | Seconds | Seconds | Seconds        | 00-59 |
| 7FF8h     | W       | R          | 10 Century | 10 Century | Century | Century | Century | Century | Control        | 00-39 |
| 7FF7h     | WDS     | BMB4       | BMB3       | BMB2       | BMB1    | BMB0    | RB1     | RB0     | Watchdog       | -     |
| 7FF6h     | AE      | Y          | ABE        | Y          | Y       | Y       | Y       | Y       | Interrupts     | -     |
| 7FF5h     | AM4     | Y          | 10 Date    | 10 Date    | Date    | Date    | Date    | Date    | Alarm Date     | 01-31 |
| 7FF4h     | AM3     | Y          | 10 Hours   | 10 Hours   | Hours   | Hours   | Hours   | Hours   | Alarm Hours    | 00-23 |
| 7FF3h     | AM2     | 10 Minutes | 10 Minutes | 10 Minutes | Minutes | Minutes | Minutes | Minutes | Alarm Minutes  | 00-59 |
| 7FF2h     | AM1     | 10 Seconds | 10 Seconds | 10 Seconds | Seconds | Seconds | Seconds | Seconds | Alarm Seconds  | 00-59 |
...
ABE = Alarm in Battery-Backup Mode Enable AM1-AM4 = Alarm Mask Bits WF = Watchdog Flag AF = Alarm Flag 0 = 0 (Read Only) BLF = Battery Low Flag RB0 to RB1 = Watchdog Resolution Bits
...
The  DS1554  and  DS9034PCX  are  each  individually  tested  for  accuracy.  Once  mounted  together,  the module is will typically keep time accuracy to within  1.53 minutes per month (35 ppm) at 25°C. The electrical environment affects clock accuracy. Caution should be taken to place the RTC in the lowestlevel EMI section of the PC board layout. For additional information, refer to Application Note 5 8 .
...
The  alarm  settings  and  control  for  the  DS1554  reside  within  Registers  7FF2h-7FF5h.  Register  7FF6h contains two alarm enable bits: Alarm Enable (AE) and Alarm in Backup Enable (ABE). The AE and ABE bits must be set  as  described  below  for  the  IRQ /FT  output  to  be  activated  for  a  matched  alarm condition.
...
When WDS is set to a 1, the watchdog will output a negative pulse on the RST output for a duration of 40 ms to 200 ms. The Watchdog Register (7FF7) and the FT bit will reset to a 0 at the end of a watchdog time-out when the WDS bit is set to a 1.
...
| PARAMETER                                 | SYMBOL   | CONDITIONS                                            | MIN   | TYP   | MAX   | UNITS   |
|-------------------------------------------|----------|-------------------------------------------------------|-------|-------|-------|---------|
| Active Supply Current                     | I CC     | (Notes 2, 3, 11)                                      |       | 40    | 75    | mA      |
...
| PARAMETER                                | SYMBOL   | CONDITIONS                                           | MIN           | TYP           | MAX           | UNITS   |
|------------------------------------------|----------|------------------------------------------------------|---------------|---------------|---------------|---------|
| Active Supply Current                    | I CC     | (Notes 2, 3, 11)                                     |               | 10            | 30            | mA      |
| TTL Standby Current (CE = V IH )         | I CC1    | (Notes 2, 3)                                         |               | 0.7           | 6             | mA      |
| CMOS Standby Current (CE  V CC - 0.2V) | I CC2    | (Notes 2, 3)                                         |               | 0.7           | 4             | mA      |
| Input Leakage Current (Any Input)        | I IL     |                                                      | -1            |               | +1            |  A     |
| Output Leakage Current (Any Output)      | I OL     |                                                      | -1            |               | +1            |  A     |
| Output Logic 1 Voltage (I OUT = -1.0 mA) | V OH     | (Note 1)                                             | 2.4           |               |               | V       |
| Output Logic 0 Voltage                   | V OL1    | I OUT = 2.1 mA, DQ0-7 Outputs (Note 1)               |               |               | 0.4           | V       |
...
| PARAMETER                                    | SYMBOL   | CONDITIONS   |   MIN | TYP   |   MAX | UNITS   |
|----------------------------------------------|----------|--------------|-------|-------|-------|---------|
| CE orWE at V IH , Before Power-Down          | t PD     |              |     0 |       |       |  s     |
...
- 7) Each DS1554 has a built-in switch that disconnects the lithium source until V CC is first applied by the user. The expected t DR is defined for DIP modules and PowerCap modules as a cumulative time in the absence of VCC starting from the time power is first applied by the user.
