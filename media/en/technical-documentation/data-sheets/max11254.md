...
The MAX11254 offers a 6.2nV/ √ Hz noise programmable gain amplifier (PGA) with gain settings from 1x to 128x. The integrated PGA provides isolation of the signal inputs from the switched capacitor sampling network. The PGA also  enables  the  MAX11254  to  interface  directly  with high-impedance sources without compromising available dynamic range.
...
| AIN_P, AIN_N,REFP, REFN, CAPP, CAPN to AVSS                       | ............................ -0.3V to the lower of                                          |
| GPO_ to GPOGND....................................                | +3.9V or (V AVDD + 0.3V) -0.3V to the lower of                                              |
| CAPREG to                                                         | +3.9V or (V AVDD + 0.3V) AVSS................................................-0.3V to +3.9V |

## 24-Bit, 6-Channel, 64ksps, 6.2nV/¥Hz PGA, Delta-Sigma ADC with SPI Interface

| CAPREG to DGND...............................................-0.3V to +2.1V                                             |
|-------------------------------------------------------------------------------------------------------------------------|
| All Other Pins to DGND.............................. -0.3V to the lower of +3.9V or (V + 0.3V)                          |
| DVDD Maximum Continuous Current into Any Pins Except GPOGND Pin...................................................±50mA |
...
| Noise Voltage (Referred to Input)                 |                                                   | PGAgain of 128, single-cycle mode at 1ksps data rate    | PGAlow- noise mode                                  |                                                   | 0.19                                              |                                                   | µV RMS                                            |
| Noise Voltage (Referred to Input)                 |                                                   | PGAgain of 128, single-cycle mode at 1ksps data rate    | PGAlow- power mode                                  |                                                   | 0.26                                              |                                                   | µV RMS                                            |
| Noise Voltage (Referred to Input)                 | V n                                               | PGAgain of 128, single-cycle mode at 12.8ksps data rate | PGAlow- noise mode                                  |                                                   | 0.83                                              |                                                   | µV RMS                                            |
| Noise Voltage (Referred to Input)                 | V n                                               | PGAgain of 128, single-cycle mode at 12.8ksps data rate | PGAlow- power mode                                  |                                                   | 1.16                                              |                                                   | µV RMS                                            |
| Noise Voltage (Referred to Input)                 | V n                                               | PGAgain of 128, continuous mode at 64ksps data rate     | PGAlow- noise mode                                  |                                                   | 0.83                                              |                                                   | µV RMS                                            |
| Noise Voltage (Referred to Input)                 | V n                                               | PGAgain of 128, continuous mode at 64ksps data rate     | PGAlow- power mode                                  |                                                   | 1.16                                              |                                                   | µV RMS                                            |
...
The MAX11254 measures six pairs of differential analog inputs  (AIN\_P,  AIN\_N)  in  direct  connection  or  buffered through the PGA.
...
The ADC input range is programmable for bipolar (-V REF to +V REF ) or unipolar (0 to VREF) ranges. The U/ B bit in the CTRL1 register configures the MAX11254 for unipolar or bipolar transfer functions.

## Data Rates

Table 1 lists the available data rates for the MAX11254, RATE[3:0]  setting  of  the  conversion  command  (see  the Modes and Registers section). The single-cycle mode has an overhead of 48 digital master clocks that is approximately 5.86µs for a typical digital master clock frequency of 8.192MHz. The single-cycle effective column contains the data rate values including the 48 clock startup delays. The 48 clocks are required to stabilize the modulator at startup. In continuous conversion mode, the output data rate is five times the single-cycle rate up to a maximum of  64ksps.  During  continuous  conversions,  the  output sample data requires five 24-bit cycles to settle to a valid conversion from an input step, PGA gain changes, or a change of input channel through the multiplexer.
...
In continuous mode, RDYB remains high for the first four conversion results and on the 5th result, RDYB goes low.
...
|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| MODE[1:0]  | The MODE bits are used to set the functional operation of the MAX11254 according to the following decoding.                                                                                                                | The MODE bits are used to set the functional operation of the MAX11254 according to the following decoding.                                                                                                                | The MODE bits are used to set the functional operation of the MAX11254 according to the following decoding.                                                                                                                |
...
| RATE[3:0]  | These bits determine the conversion speed of the MAX11254. The decoding is shown in Table 1.                                                                                                                               | These bits determine the conversion speed of the MAX11254. The decoding is shown in Table 1.                                                                                                                               | These bits determine the conversion speed of the MAX11254. The decoding is shown in Table 1.                                                                                                                               |
...
CSB=0; SPI=0xD3000000; CSB=1; Wait
...
CSB=0; SPI=0xD5000000; CSB=1; Wait
...
CSB=0; SPI=0xD1000000; CSB=1; STOP

Read register DATA0; Mode activity is completed. The MAX11254 powers down into SLEEP state waiting for the next command
...
To  prevent  ambiguous  power-supply  conditions  from causing erratic behavior, voltage detectors monitor AVDD, DVDD, and CAPREG and hold the MAX11254 in reset when supplies fall below V HL  (see Figure 13). The analog  undervoltage  lockout  (AVDD  UVLO)  prevents  the ADC from converting when AVDD falls below V HL . The CAPREG  UVLO  resets  and  prevents  the  low-voltage digital logic from operating at voltages below V HL . DVDD UVLO thresholds supersede CAPREG thresholds when CAPREG is externally driven. Figure 14 shows a flow diagram of the POR sequence. Glitches on supplies AVDD, DVDD, and CAPREG for durations shorter than T P  are suppressed without triggering  POR or UVLO. For glitch durations  longer  than T P ,  POR  is  triggered  within T DEL seconds. See the Electrical Characteristics table for values of V LH , V HL , T P , and T DEL .
...
| COMMAND ISSUED*   | MAX11254 STATE BEFORE COMMAND   | COMMAND INTERPRETATION                                                                                                                   | MAXIMUM DELAYTIMETO NEXT STATE †   | MAX11254 STATE AFTER COMMAND   |
|-------------------|---------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|--------------------------------|
| SLEEP             | RESET                           | Command ignored                                                                                                                          | 0                                  | RESET                          |
| SLEEP             | SLEEP                           | Command ignored                                                                                                                          | 0                                  | SLEEP                          |
| SLEEP             | STANDBY                         | MAX11254 powers down into SLEEP mode                                                                                                     | 20ms                               | SLEEP                          |
...
| SLEEP             | Calibration                     | Calibration stops, MAX11254 powers down into SLEEP mode                                                                                  | 3µs                                | SLEEP                          |
| SLEEP             | Conversion                      | Conversion stops, MAX11254 powers down into SLEEP mode                                                                                   | 3µs                                | SLEEP                          |
...
| STANDBY           | SLEEP                           | MAX11254 changes to STANDBY                                                                                                              | 20ms                               | STANDBY                        |
...
## 24-Bit, 6-Channel, 64ksps, 6.2nV/¥Hz PGA, Delta-Sigma ADC with SPI Interfac
...
The MAX11254 supports unipolar and split analog power supplies  for  input  range  flexibility.  Using  a  split  analog supply  enables  sampling  below  ground  reference.  The true bipolar input range is up to ±1.8V. See Figure 3 for analog  input  voltage  range  for  both  unipolar  and  split supplies.

## Sensor Fault Detection

The  MAX11254  includes  a  1µA  current  source  and  a 1µA current sink. The source pulls current from AVDD to AIN\_P and sink from AIN\_N to AVSS. The currents are enabled  by  register  bit  CTRL3:CSSEN.  These  currents are  used  to  detect  damaged  sensors  in  either  open  or shorted  state.  The  current  sources  and  sinks  are  functional over the normal input operating voltage range, as specified.
...
| PDSTAT[1:0] | These bits indicate the state of the MAX11254. See Table 6 for transition times.                                                                                                                                                                                                                        | These bits indicate the state of the MAX11254. See Table 6 for transition times.                                                                                                                                                                                                                        | These bits indicate the state of the MAX11254. See Table 6 for transition times.                                                                                                                                                                                                                        |
...
| PD[1:0]    | Selects the power-down state to be executed. The MAX11254 enters the selected power-down state after a conversion command with MODE[1:0] set to '01' is written. The state is decoded as below:                                                                                                                                                                                                                                                | Selects the power-down state to be executed. The MAX11254 enters the selected power-down state after a conversion command with MODE[1:0] set to '01' is written. The state is decoded as below:                                                                                                                                                                                                                                                | Selects the power-down state to be executed. The MAX11254 enters the selected power-down state after a conversion command with MODE[1:0] set to '01' is written. The state is decoded as below:                                                                                                                                                                                                                                                |
...
|                 2 | 8/17            | Adding MAX11254ATJ/V+ as future product to Ordering Information table. Updated Package Information table | 50              |
