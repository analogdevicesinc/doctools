...
- 9) Set the input voltage above 6V.
- 10)  Connect the DVM across the RESET pad and SGND. Verify that the DVM displays 5V.
- 11)  Reduce the input voltage to 3.5V which is below the EN/UVLO falling threshold.
- 12)  Connect the DVM across the VOUT pad and nearest PGND. Verify that the DVM displays 0V.
- 13)  Connect the DVM across the RESET pad and SGND. Verify that the DVM displays 0V.
...
The MAX17634AEVKIT# is designed to demonstrate the salient features of the MAX17634A. The EV kit includes an EN/UVLO pad and jumper JU1 to enable the output at a desired input voltage. The Jumper JU2 allows selection of a particular mode of operation based on light load performance requirements. The MODE/SYNC pin on jumper JU2 allows an external clock interface to synchronize the device. An additional RESET pad is available for monitoring the status of the output voltage.
...
The  MAX17634  offers  an  Enable  and  adjustable  input undervoltage  lockout  feature.  In  this  EV  kit,  for  normal operation, leave EN/UVLO jumper (JU1) open. When JU1 is  left  open,  the  MAX17634  is  enabled  when  the  input voltage  rises  above  4.4V.  To  disable  MAX17634,  install a jumper across pins 2-3 on JU1. See Table 1 for JU1 settings. The EN/UVLO PCB pad on the EV kit supports external Enable/Disable control of the device. Leave JU1 open when external Enable/Disable control is desired. A potential divider formed by R1 and R2 sets the input voltage (V INU )  above  which  the  converter  is  enabled  when JU1 is left open.

## Evaluates: MAX17634A 3.3V Output-Voltage Application
...
| SHUNT POSITION   | EN/UVLO PIN                                                | MAX17634A EV KIT OUTPUT                                 |
|------------------|------------------------------------------------------------|---------------------------------------------------------|
...
| SHUNT POSITION   | MODE/SYNC PIN       | MAX17634A EV KIT OUTPUT   |
|------------------|---------------------|---------------------------|
...
## MAX17634AEVKIT# Performance Report (continued)
...
Note: Indicate that you are using the MAX17634A IC when contacting these component suppliers.

│

## MAX17634AEVKIT# Bill of Materials
...
|      9 | L1                | INDUCTOR, 5.6µH, 20%, 9.9A (6mm x 6mm)                                       |          1 | COILCRAFT XAL6060-562ME    |
|     10 | R1                | RESISTOR, 3.32MΩ, 1% (0402)                                                  |          1 | Any                        |
|     11 | R2                | RESISTOR, 1.30MΩ, 1% (0402)                                                  |          1 | Any                        |
|     12 | R3, R8            | RESISTOR, 0Ω (0402)                                                          |          2 | Any                        |
|     13 | R5                | RESISTOR, 51kΩ, 1% (0402)                                                    |          1 | Any                        |
|     14 | R6                | RESISTOR, 10kΩ, 1% (0402)                                                    |          1 | Any                        |
|     15 | U1                | HIGH-EFFICIENCY; SYNCHRONOUS STEP-DOWN DC-DC CONVERTER (TQFN20-EP 4mm x 4mm) |          1 | MAX17634AATP+              |
...
## MAX17634AEVKIT# Schematic

<!-- image -->

## MAX17634AEVKIT# Evaluation Kit

## MAX17634AEVKIT# PCB Layout

MAX17634A EV Kit-Top Silkscreen

<!-- image -->

MAX17634A EV Kit-Top

<!-- image -->

## Evaluates: MAX17634A 3.3V Output-Voltage Application

MAX17634A EV Kit-Layer2 GND

<!-- image -->

MAX17634A EV Kit-Layer3 GND

<!-- image -->

│

## MAX17634A EV Kit PCB Layout (continued)

MAX17634A EV Kit-Bottom

<!-- image -->

## Evaluates: MAX17634A 3.3V Output-Voltage Application

MAX17634A EV Kit-Bottom Silkcreen
