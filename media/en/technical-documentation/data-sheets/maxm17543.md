...
## . V to   V,  . A High-Eႈciency, DC-DC StepDoZn PoZer Module Zith ,ntegrated ,nductor
...
- Integrated Inductor
- Integrated Compensation Component
...
| Storage Temperature Range.............................-65ºC to +125°C                |
...
Note 3: Data taken using Maxim's evaluation kit, MAXM17543EVKIT#.

## Electrical Characteristics

(V IN  = V EN  = 24V, R RT = 40.2kΩ (500kHz) to SGND, V PGND = V MODE  = V SYNC  = V SGND  = 0V, V CC  = LX = SS = RESET = OUT = open, V BST  to V LX  = 5V, V FB  = 1V, T A  = T J  = -40ºC to +125ºC, unless otherwise noted. Typical values are at T A  = +25°C. All voltages are referenced to SGND, unless otherwise noted.) (Note 4)
...
(V IN  = V EN  = 24V, R RT = 40.2kΩ (500kHz) to SGND, V PGND = V MODE  = V SYNC  = V SGND  = 0V, V CC  = LX = SS = RESET = OUT = open, V BST  to V LX  = 5V, V FB  = 1V, T A  = T J  = -40ºC to +125°C, unless otherwise noted. Typical values are at T A  = +25°C. All voltages are referenced to SGND, unless otherwise noted.) (Note 4)
...
| MODE Threshold                                          | V M_PFM     | MODE = open (PFM mode)             | V CC /2    | V CC /2 |       | V       |
| MODE Threshold                                          | V M_PWM     | MODE = GND (PWM mode)              | 1.4        | 1.4     |   1.4 | V       |
| CURRENT LIMIT                                           |             |                                    |            |         |       |         |
| Average Current-Limit Threshold                         | I AVG_LIMIT | V OUT = V FB = 0.8V, f SW = 200kHz | 3.45       | 3.45    |       | A       |
| RESET                                                   |             |                                    |            |         |       |         |
...
| RESET Deassertion Delay After FB Reaches 95% Regulation |             |                                    | 1024       | 1024    |       | Cycles  |
| THERMAL SHUTDOWN                                        |             |                                    |            |         |       |         |
...
## 4.5V to 42V, 2.5A High-Eႈciency, DC-DC Step-Down Power Module with Integrated Inductor
...
RE 30MHz-1GHz\_0-360 deg\_90deg step\_1-4mtr Height\_Quick scan\_PI65\_Test8.TIL
...
## 2 column (6.96 in.) 4.5V to 42V, 2.5A High-Eႈciency, DC-DC Step-Down Power Module with Integrated Inductor
...
| 8      | MODE   | Light-Load Mode Selection. The MODE pin configures the MAXM17543 to operate in PWM, PFM, or DCM mode of operation. Leave MODE unconnected for PFM operation (pulse skipping at light loads). Connect MODE to SGND for constant-frequency PWM operation at all loads. Connect MODE to V CC for DCM operation. See the MODE Selection (MODE) section for more details. |
...
## 4.5V to 42V, 2.5A High-Eႈciency, DC-DC Step-Down

## Power Module with Integrated Inductor

f SW = Selected operating switching frequency in Hz

t OFF\_MIN(MAX)  = Worst-case minimum switch off-time (160ns) t ON\_MIN(MAX)  = Worst-case minimum switch on-time (80ns)
...
| 4.8 to 2 8 |         3.3 | 2 x 2.2µF 1206 100V | 1 x 47µF 1210 10V          |        130 | 48.7       |          500 | Open       |
...
| 4.8 to 42  |         3.3 | 2 x 2.2µF 1206 100V | 1 x 47µF 1210 10V          |        130 | 48.7       |          500 | Open       |
...
The switching frequency range of 100kHz to 1.8MHz are recommended from Table 1 for desired input and output voltages. The switching frequency of MAXM17543 can be programmed by using a single resistor (R RT ) connected from the RT pin to SGND. The calculation of R RT  resistor is given by the following equation:
...
The  device  can  be  synchronized  by  an  external  clock signal  on  the  SYNC  pin.  The  external  synchronization clock  frequency  must  be  between  1.1  x  f SW   and  1.4  x
...
The  MAXM17543 is  provided  with  a  robust  overcurrent protection (OCP) scheme that protects the module under overload  and  output  short-circuit  conditions. A  cycle-by -cycle peak current limit turns off the high-side MOSFET whenever the high-side switch current exceeds an internal  limit  of  3.7A  (typ).  The  module  enters  hiccup  mode of  operation either after one occurrence of the runaway current limit 4.3A (typ) or when the FB node goes below 0.58V of  its  nominal  regulation  threshold  after  soft-start is  complete.  In  hiccup  mode,  the  module  is  protected by  suspending  switching  for  a  hiccup  timeout  period  of 32,768  clock  cycles.  Once  the  hiccup  timeout  period expires,  soft-start  is  attempted  again.  Hiccup  mode  of operation  ensures  low  power  dissipation  under  output overload or short-circuit conditions. Note that when soft -start  is  attempted  under  overload  condition,  if  feedback voltage  does  not  exceed  0.58V,  the  device  switches  at half the programmed switching frequency.

The MAXM17543 is designed to support a maximum load  current  of  2.5A.  The  inductor  ripple  current  is calculated as follows:
...
## 9 WR   9     $ +LJη-(ႈFLH 3RZHU 0RGXOH ZLWη ,QW
...
## . V to   V,  . A High-Eႈciency, DC-DC Step-DoZn PoZer Module Zith ,ntegrated ,nductor
...
|                 1 | 11/16           | Updated Package Thermal Characteristics and notes sections, updated Pin 4 in the Pin Description section, and updated the Loop Compensation section                                                           | 2, 10, 14       |
