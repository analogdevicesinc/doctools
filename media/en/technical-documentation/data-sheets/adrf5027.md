...
## Silicon SPDT Switch, Nonreflective, 9 kHz to 44 GHz

[ADRF5027](http://www.analog.com/adrf5027?doc=ADRF5027.pdf)
...
The ADRF5027 comes in a 20-terminal, 3 mm × 3 mm, RoHScompliant, land grid array (LGA) package and can operate from -40°C to +105°C.
...
| REVISION HISTORY 9/2020-Rev. 0 to Rev.A                                                                                                                               |
| Changes to Table 2........................................................................... 5                                                                       |
| Changes to Theory of Operation Section...................................... 9 Changes to Ordering Guide.......................................................... 12 |
...
|                           |                 | 18 GHz to 26 GHz                                   |       | 28    |        | dB     |
|                           |                 | 26 GHz to 35 GHz                                   |       | 17    |        | dB     |
|                           |                 | GHz to 40 GHz                                      |       | 13    |        | dB     |
|                           |                 | 35                                                 |       |       |        |        |
|                           |                 | 40 GHz to 44 GHz 1                                 |       | 10    |        | dB     |
| ISOLATION                 |                 |                                                    |       |       |        |        |
...
| Rise and Fall Time        | t RISE , t FALL | 10% to 90% of RF output                            |       | 1.3   |        | µs     |
| On and Off Time           | t ON , t OFF    | 50% of triggered V CTRL to 90% of RF output        |       | 3.6   |        | µs     |
| RF Settling Time          |                 |                                                    |       |       |        |        |
| 0.1 dB                    |                 | 50%oftriggeredV CTRL to 0.1 dBoffinal RF output    |       | 6.0   |        | µs     |
| 0.05 dB                   |                 | 50%oftriggeredV CTRL to 0.05 dBoffinalRF output    |       | 7.5   |        | µs     |
| INPUT LINEARITY 2         |                 | 200 kHz to 40 GHz                                  |       |       |        |        |
| 1 dB Compression          | P1dB            |                                                    |       | 26    |        | dBm    |
| Third-Order Intercept     | IP3             | Two-tone input power = 12 dBmeach tone, ∆f = 1 MHz |       | 54    |        | dBm    |
| SUPPLY CURRENT            |                 | VDD and VSS pins                                   |       |       |        |        |
| Positive                  | I DD            |                                                    |       | 2     |        | µA     |
|                           |                 |                                                    |       |       |        | µA     |
| DIGITAL CONTROL INPUTS    |                 |                                                    |       |       |        |        |
|                           | SS              |                                                    |       |       |        |        |
| Negative Voltage          | I               |                                                    |       | 100   |        |        |
|                           |                 | CTRL and EN pins                                   |       |       |        |        |
| Low                       | V INL           |                                                    |     0 |       |    0.8 | V      |
...
| Digital Control Voltage         | V CTRL , V EN |                                                                     |     0 |       | V DD  | V      |
| RF Input Power 3                | P IN          | f = 200 kHz to 40 GHz, T CASE = 85°C 4                              |       |       |       |        |
...
| Parameter                                                                                                                                                                | Rating                            |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|
| Positive Supply Voltage Negative Supply Voltage Digital Control Inputs Voltage Current RF Input Power 1 (200 kHz to T CASE = 85°C 2 ) Insertion Loss Path Isolation Path | -0.3 V to +3.6 V -3.6 V to +0.3 V |
|                                                                                                                                                                          | -0.3VtoVDD+0.3V 3mA               |
| 40 GHz at Hot Switching                                                                                                                                                  | 26dBm 25dBm 25dBm                 |
| RF Input Power Under                                                                                                                                                     |                                   |
| Unbiased Condition (V DD , V SS = 0 V)                                                                                                                                   | 20dBm                             |
| Temperature Junction, T J Storage Range                                                                                                                                  | 135°C -65°C to +150°C             |
| Reflow Electrostatic Discharge (ESD) Sensitivity Human Body Model (HBM) RFC, RF1, RF2 Pins                                                                               | 260°C 1500 V 2000 V               |
| Digital Pins                                                                                                                                                             |                                   |
...
Insertion loss and return loss are measured on the probe matrix board using ground-signal-ground (GSG) probes close to the RFx pins. Signal coupling between the probes limits the isolation performance of ADRF5027. Isolation is measured on the ADRF5027-EVALZ evaluation board. See the Applications Information section for details on the ADRF5027-EVALZ evaluation board and probe matrix board.
...
VDD = 3.3 V, VSS = -3.3 V, VCTRL/VEN = 0 V or VDD, and TCASE = 25°C in a 50 Ω system, unless otherwise noted. All of the large signal performance parameters are measured on the ADRF5027-EVALZ evaluation board.
...
The ADRF5027 supports an all off state control. When the EN pin is logic high, both the RF1 to RFC path and the RF2 to RFC path are in an isolation state, regardless of the logic state of the CTRL pin. The RF1 and RF2 ports are terminated to internal 50 Ω resistors, and the RFC port becomes open reflective (see Table 5).

Table 5. Control Voltage Truth Table

The ADRF5027 design is bidirectional with equal power handling capabilities. An RF input signal (RFIN) can be applied to the RFC port or the RF1 or RF2 port. The isolation path provides high loss between the unselected RFx port and the insertion loss path.

The ideal power-up sequence is as follows:

1. Connect GND.
2. Power up VDD and VSS. Power up VSS after VDD to avoid current transients on VDD during ramp-up.
3. Apply the digital control inputs. The relative order of the digital control inputs is not important. However, powering the digital control inputs before the VDD supply may inadvertently forward bias and damage the internal ESD protection structures. To avoid this damage, use a series 1 kΩ resistor to limit the current flowing in to the control pin. Use pull-up or pull-down resistors if the controller output is in a high impedance state after VDD is powered up and the control pins are not driven to a valid logic state.
...
The ADRF5027-EVALZ evaluation board is a 4-layer evaluation board. The outer copper (Cu) layers are 0.5 oz (0.7 mil) plated to 1.5 oz (2.2 mil) and are separated by dielectric materials. Figure 15 shows the ADRF5027-EVALZ evaluation board stack up.
...
The thru calibration line, THRU CAL, can calibrate out the board loss effects from the ADRF5027-EVALZ evaluation board measurements to determine the device performance at the pins of the IC. Figure 17 shows the typical board loss for the ADRF5027-EVALZ evaluation board at room temperature, the embedded insertion loss, and the de-embedded insertion loss for the ADRF5027.
...
The ADRF5027-EVALZ evaluation board schematic is shown in Figure 18.
...
Narrow-band impedance matching on the RF transmission lines can improve return loss and insertion loss for a targeted frequency range. The impedance matched circuit, highlighted in Figure 22, achieves a low insertion loss response of 2.2 dB from 28 GHz to 43 GHz (see Figure 23). The dimensions of the 50 Ω lines are 14 mil trace width and a 7 mil gap. To implement this impedance matched circuit, an 8 mil trace with a width of 5 mil is inserted between the pin pad and the 50 Ω trace.

Table 7, Figure 23, Figure 24, and Figure 25 show the measured performance of ADRF5027 on the impedance matched circuit on the probe matrix board.
...
|------------------------|-------------------------------|-------|--------|
| Insertion Loss Between | See Figure 23 9 kHz to 18 GHz |       |        |
| RFCandRF1/RF2          |                               |   1.1 | dB     |
|                        | 18 GHz to 26 GHz              |   1.8 | dB     |
|                        | 26 GHz to 35 GHz              |   2.3 | dB     |
|                        | 35 GHz to 40 GHz              |   2.2 | dB     |
|                        | 40 GHz to 44 GHz              |   2.7 | dB     |
| Return Loss            | See Figure 24                 |       |        |
| RFCandRF1/RF2(On)      | 9 kHz to 18 GHz               |    20 | dB     |
...
|----------------------------------------------------------------------|---------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|------------------|----------------|
| ADRF5027BCCZN ADRF5027BCCZN-R7 ADRF5027-EVALZ-292 ADRF5027-EVALZ-240 | -40°C to +105°C -40°C to +105°C | 20-Terminal Land Grid Array [LGA] 20-Terminal Land Grid Array [LGA] Evaluation Board with 2.92 mmconnectors Evaluation Board with 2.40 mmconnectors | CC-20-3 CC-20-3  | 027 027        |
