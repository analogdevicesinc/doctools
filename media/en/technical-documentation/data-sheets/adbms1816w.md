...
- ±3.6 mV maximum total measurement error
...
- Sleep mode supply current: 26 µA
- 64-lead LFCSP\_SS with side solderable leads
- AEC-Q100 qualified for automotive application

## APPLICATIONS

- Electric and hybrid electric vehicles
- Backup battery systems and UPS
- Grid energy storage
...
The ADBMS1816W is a multicell battery stack monitor that measures up to 16 series connected battery cells with a total measurement error of less than 3 mV after software calibration. The cell measurement range of 0 V to 4.5 V makes the ADBMS1816W suitable for most battery chemistries. A programmable gain amplifier (PGA) is integrated to allow for the monitoring of various currents based on the target current range in the application. All 16 cells, stack voltage, and current can be measured in 116.8 μs.

Multiple ADBMS1816W devices can be connected in series, permitting simultaneous cell monitoring of long, high-voltage battery strings. Each ADBMS1816W has an isolated, serial peripheral interface (isoSPI) for high-speed, RF immune, and long-distance communication.
...
The ADBMS1816W can be powered directly from the battery stack or from an isolated supply. The ADBMS1816W includes passive balancing for each cell, with individual pulse-width modulation (PWM) duty-cycle control for each cell. The ADBMS1816W has a dedicated ALERT output pin to indicate the fault state. Other features include an on-board 5 V regulator and a sleep mode, where the supply current consumption is reduced to 26 µA.

For more information about the ADBMS1816W, contact svc\_CBMS\_Doc\_Req@analog.com.
