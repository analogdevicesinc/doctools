...
xDSL Instrumentation Medical Imaging Scanners IR Imaging
...
|------------------------------------------------------------------------------------------------------|
| Operating Temperature Ranges (T A )                                                                  |
...
|------------------------------------------|----------|-------------------------------|--------|--------|-------|-----------------|
| Input Voltage Range (Notes 2, 3)         | V IN     | Single-ended                  |        | 4.096  | 4.5   | V               |
|                                          |          | Differential                  |        | ±4.096 | ±4.5  |                 |
| Input Resistance (Note 4)                | R I      |                               |        | 25     |       | k Ω             |
...
| Input-Referred Noise                     |          | f CLK                         |        | 75     |       | µV RMS          |
| Maximum Sampling Rate                    | f SAMPLE | f SAMPLE = f CLK/2            | 2.2528 |        |       | Msps            |
...
| Full-Power Bandwidth                     |          |                               |        | 78 3.3 |       | MHz             |
...
| PARAMETER                                   | SYMBOL             | CONDITIONS                   | MIN                | TYP                | MAX                | UNITS              |
|---------------------------------------------|--------------------|------------------------------|--------------------|--------------------|--------------------|--------------------|
| Signal-to-Noise Ratio (Note 5)              | SNR                | f IN = 100.1kHz              | 78                 | 83                 |                    | dB                 |
|                                             |                    | f IN = 502.7kHz              |                    | 82                 |                    | dB                 |
|                                             |                    | f IN = 1.0021MHz             |                    | 81                 |                    | dB                 |
| Spurious-Free Dynamic Range (Note 5)        | SFDR               | f IN = 100.1kHz              | 84                 | 91                 |                    | dB                 |
|                                             |                    | f IN = 502.7kHz              |                    | 89                 |                    | dB                 |
|                                             |                    | f IN = 1.0021MHz             |                    | 86                 |                    | dB                 |
| Total Harmonic Distortion (Note 6)          | THD                | f IN = 100.1kHz              |                    | -88                | -80                | dB                 |
|                                             |                    | f IN = 502.7kHz              |                    | -85                |                    | dB                 |
|                                             |                    | f IN = 1.0021MHz             |                    | -83                |                    | dB                 |
| Signal-to-Noise Ratio + Distortion (Note 5) | SINAD              | f IN = 100.1kHz              | 77                 | 82                 |                    | dB                 |
| Signal-to-Noise Ratio + Distortion (Note 5) | SINAD              | f IN = 502.7kHz              |                    | 79                 |                    | dB                 |
| Signal-to-Noise Ratio + Distortion (Note 5) | SINAD              | f IN = 1.0021MHz             |                    | 78                 |                    | dB                 |
...
| Analog Supply Current                       | I(AV DD )          |                              |                    | 53                 | 75                 | mA                 |
| Digital Supply Voltage                      | DV DD              |                              | 3                  |                    | 5.25               | V                  |
| Digital Supply Current                      | I(DV DD )          |                              |                    | 1                  | 2                  | mA                 |
| Output Drive Supply Voltage                 | DRV DD             |                              | 3                  |                    | DV DD              | V                  |
| Output Drive Supply Current                 | I(DRV DD )         | 10pF loads on D0-D13 and DAV |                    | 0.3                | 1                  | mA                 |
...
| Clock Period           | t CLK    |                | 227        | 227        | 227        | ns           |
| Clock High Time        | t CH     |                | 85         | 113.5      | 137        | ns           |
| Clock Low Time         | t CL     |                | 85         | 113.5      | 137        | ns           |
...
(AVDD = +5V, DVDD = DRVDD = +3.3V, TA = TMIN to TMAX, unless otherwise noted. Typical values are at TA = +25°C.)
...
| CLK Input Current              | I CLK     | V IN_ = 0 or DV DD |             | ±1           | ±10   | µA      |
| CLK Input Capacitance          | CCLK      |                    |             | 9            |       | pF      |
| Digital Input Current          | I IN      | V IN_ = 0          |             | ±0.1         | ±10   | µA      |
| Digital Input Current          | I IN      | V IN_ = DV DD      |             | ±0.1         | ±10   | µA      |
...
Note 2: For unipolar mode, the analog input voltage, VINP, must be within 0V and VREF, VINN = VREF /2; where VREF = VRFPS -

VRFNS. For differential mode, the analog input voltages VINP and VINN must be within 0V and VREF; where VREF = VRFPS -

VRFNS. The common-mode voltage of the inputs INP and INN is VREF /2.
...
The switched capacitor circuit at the analog input allows signals between AGND and the analog power supply. Since the common-mode voltage has a strong influence on the performance of the ADC, the best results are obtained by choosing VCM to be at half the difference between the reference voltages VRFP and VRFN. Achieve this  by  using  a  resistive  divider  between the two reference potentials. Figure 4 shows the driving circuit for good dynamic performance.
...
For example, if fIN is  1MHz and s JITTER is 10ps RMS, then the SNR limit due to jitter is approximately 84dB. Generating such a clock source requires a low-noise comparator and a low-phase noise signal generator. The clock circuit shown in Figure 6 is a possible solution.
...
Once calibrated, the MAX1201 is insensitive to small changes (±5%) in power supply, voltage, or temperature.  Following  calibration,  if  the  temperature  changes more than ±20°C, the device should be recalibrated to maintain optimum performance.
