<!-- lastmod 2022-08-02 -->
## MAX17544 3.3V Output Evaluation Kit

## General Description

The MAX17544 3.3V output evaluation kit (EV kit) provides a proven design to evaluate the MAX17544 high-voltage, high-efficiency,  synchronous  step-down  DC-DC converter. The EV kit is preset for 3.3V output at load currents up to 3.5A and features a 450kHz switching frequency for optimum  efficiency  and  component  size.  The  EV  kit features  adjustable  input  undervoltage  lockout,  adjustable  soft-start,  open-drain RESET signal,  and  external frequency synchronization.
...
|---------------|-------|-----------------------------------------------------------------------------|
| C1, C8        |     2 | 2.2µF ±10%, 50V X7R ceramic capacitors (1210) TDK C3225X7R1H225K            |
| C2            |     1 | 2.2µF ±10%, 10V X7R ceramic capacitor (0603) Murata GRM188R71A225K          |
| C3            |     1 | 12000pF ±10%, 16V X7R ceramic capacitor (0402) Murata GRM155R71C123K        |
| C4, C9        |     2 | 22µF ±10%, 10V X7R ceramic capacitors (1210) Murata GRM32ER71A226K          |
...
## Evaluates: MAX17544 in 3.3V Output-Voltage Application
...
| DESIGNATION   |   QTY | DESCRIPTION                                                            |
|---------------|-------|------------------------------------------------------------------------|
| L1            |     1 | 6.8µH, 5A inductor Coilcraft MSS1048-682NL Cooper Bussmann DR125-6R8-R |
| R1            |     1 | 3.32MΩ ±1% resistor (0402)                                             |
| R2            |     1 | 750kΩ ±1% resistor (0402)                                              |
| R3            |     1 | 82.5kΩ ±1% resistor (0402)                                             |
| R4            |     1 | 30.9kΩ ±1% resistor (0402)                                             |
| R5            |     1 | 45.3kΩ ±1% resistor (0402)                                             |
| R6            |     1 | 10kΩ ±1% resistor (0402)                                               |
| TP1, TP2      |     2 | Test pads                                                              |
| U1            |     1 | Buck converter (20 TQFN-EP*) Maxim MAX17544ATP+                        |
| -             |     3 | Shunts (JU1-JU3)                                                       |
| -             |     1 | PCB: MAX17544 - 3.3V OUTPUT EVKIT                                      |

Note: C7,  R1,  and  R2  are  optional  components;  R1  and  R2 are not needed if the EN/UVLO pin is permanently connected to  VIN.  The  electrolytic  capacitor  (C7)  is  required  only  when the VIN power supply is situated far from the MAX17544-based circuit. When R5 is open, the device switches at 500kHz switching frequency. The MSS1048 inductor has been used to prepare the EV kit test report.
...
| Cooper Bussmann | 636-394-2877 | www.cooperindustries.com |
| Murata Americas | 800-241-6574 | www.murataamericas.com   |
| Panasonic Corp. | 800-344-2112 | www.panasonic.com        |
| TDK Corp.       | 516-535-2600 | www.tdk.com              |

Note: Indicate that you are using the MAX17544 when contacting these component suppliers.

## Quick Start

## Recommended Equipment

- MAX17544	3.3V	output	EV	kit
- 5V	to	42V,	7A	DC	input	power	supply
...
- 2)  Connect	the	positive	terminal	of	the	power	supply	to the	 VIN	 PCB	 pad	 and	 the	 negative	 terminal	 to	 the nearest	PGND	PCB	pad.	Connect	the	positive	termi -nal	 of	 the	 3.5A	 load	 to	 the	 VOUT	 PCB	 pad	 and	 the negative	terminal	to	the	nearest	PGND	PCB	pad.
...
The  MAX17544  3.3V  output  EV  kit  provides  a  proven design  to  evaluate  the  MAX17544  high-voltage,  highefficiency, synchronous step-down DC-DC converter. The EV	kit	is	preset	for	3.3V	output	from	5V	to	42V	input	at load currents up to 3.5A and features a 450kHz switching frequency for optimum efficiency and component size.
...
The	 device's	 MODE	 pin	 can	 be	 used	 to	 select	 among PWM,	PFM,	or	DCM	modes	of	operation.	The	logic	state of	the	MODE	pin	is	latched	when	VCC	and	EN/UVLO	volt -ages	exceed	the	respective	UVLO	rising	thresholds	and all	internal	voltages	are	ready	to	allow	LX	switching.	State

## Table 1. Regulator Enable (EN/UVLO) Description (JU1)

| SHUNT POSITION   | EN/UVLO PIN                                                | MAX17544_ OUTPUT                                        |
|------------------|------------------------------------------------------------|---------------------------------------------------------|
...
changes	 on	 the	 MODE	 pin	 are	 ignored	 during	 normal operation.	Refer	to	the	MAX17544	IC	data	sheet	for	more information	on	PWM,	PFM,	and	DCM	modes	of	operation.
...
| SHUNT POSITION   | MODE PIN          | MAX17544_ MODE        |
|------------------|-------------------|-----------------------|
...
| SHUNT POSITION   | SYNC PIN                      | MAX17544_ SYNC                                       |
|------------------|-------------------------------|------------------------------------------------------|
...
Figure 1. MAX17544 3.3V Output Load and Line Regulation (PWM Mode)

<!-- image -->

Figure 2. MAX17544 3.3V Output Efficiency (PWM Mode)

<!-- image -->

Evaluates: MAX17544 in

Figure 3. MAX17544 3.3V Output Load and Line Regulation (PFM Mode)

<!-- image -->

Figure 4. MAX17544 3.3V Output Efficiency (PFM Mode)
...
Figure 5. MAX17544 3.3V Output Efficiency (DCM Mode)

<!-- image -->

## Evaluates: MAX17544 in 3.3V Output-Voltage Application

Figure 6. MAX17544 3.3V Output Full Load Bode Plot (VIN = 24V)

<!-- image -->

Figure 7. MAX17544 3.3V Output, No Load to 1.75A Load Transient (PWM Mode)
...
Figure 8. MAX17544 3.3V Output, 5mA to 1.75A Load Transient (PFM Mode)
...
Figure 9. MAX17544 3.3V Output, 50mA to 1.75A Load Transient (DCM Mode)

<!-- image -->

Figure 10. MAX17544 3.3V Output, 1.75A to 3.5A Load Transient
...
Figure 11. MAX17544 3.3V Output EV Kit Schematic

<!-- image -->

## MAX17544 3.3V Output Evaluation Kit

Figure 12. MAX17544 3.3V Output EV Kit Component Placement Guide-Component Side

<!-- image -->

Figure 13. MAX17544 3.3V Output EV Kit Component Side PCB layout

<!-- image -->

## Evaluates: MAX17544 in 3.3V Output-Voltage Application

Figure 14. MAX17544 3.3V Output EV Kit PCB LayoutInner Layer 1

<!-- image -->

Figure 15. MAX17544 3.3V Output EV Kit PCB Layout-Inner Layer 2

<!-- image -->

## MAX17544 3.3V Output Evaluation Kit

Figure 16. MAX17544 3.3V Output EV Kit PCB LayoutSolder Side

<!-- image -->

## Evaluates: MAX17544 in 3.3V Output-Voltage Application

Figure 17. MAX17544 3.3V Output EV Kit Component Placement Guide-Top Solder Mask

<!-- image -->

Figure 18. MAX17544 3.3V Output EV Kit Component Placement Guide-Bottom Solder Mask
